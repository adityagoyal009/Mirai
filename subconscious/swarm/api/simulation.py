...
Step 2: Zep entity reading/filtering, OASIS simulation preparation and execution (fully automated)
"""
...
# Adding this prefix prevents agents from calling tools, responding directly in text
...
def get_graph_entities(graph_id: str):
    ...
    Only returns nodes with predefined entity types (nodes with Labels beyond just Entity)
...
def create_simulation():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Project graph not yet built, please call /api/graph/build"
            }), 400
        ...
        logger.error(f"Creating simulation failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    # Required files list (excluding scripts, scripts located in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # 如果 config_generated=True 且文件存在，认为Preparation complete
        # 以下状态都说明准备工作已完成：
        # - ready: Preparation complete，可以运行
        ...
        if status in prepared_statuses and config_generated:
            ...
            logger.info(f"模拟 {simulation_id} 检测结果: 已Preparation complete (status={status}, config_generated={config_generated})")
            return True, {
                "status": status,
            ...
            logger.warning(f"模拟 {simulation_id} 检测结果: 未Preparation complete (status={status}, config_generated={config_generated})")
            return False, {
...
def prepare_simulation():
    """
    Prepare simulation environment（异步任务，LLM智能生成所有参数）
    
    这是一个耗时操作，API会立即返回task_id，
    使用 GET /api/simulation/prepare/status for progress
    
    特性：
    - 自动检测已完成的准备工作，避免重复生成
    - 如果已Preparation complete，直接返回已有结果
    ...
    Request (JSON):
        {
            ...
            "entity_types": ["Student", "PublicFigure"],  // Optional，指定实体类型
    ...
    try:
        ...
        # 检查是否已经Preparation complete（避免重复生成）
        if not force_regenerate:
            logger.debug(f"检查模拟 {simulation_id} 是否已Preparation complete...")
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            logger.debug(f"检查结果: is_prepared={is_prepared}, prepare_info={prepare_info}")
            if is_prepared:
                logger.info(f"模拟 {simulation_id} 已Preparation complete，跳过重复生成")
                return jsonify({
                    "success": True,
                    "data": {
                ...
                logger.info(f"模拟 {simulation_id} 未Preparation complete，将启动准备任务")
        ...
        def run_prepare():
            try:
                task_manager.update_task(
                    ...
                    progress=0,
                    message="开始Prepare simulation environment..."
                )
                
                # 准备模拟（带Progress callback）
                # 存储阶段进度详情
                stage_details = {}
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "preparing",
                "message": "准备任务已启动，请通过 /api/simulation/prepare/status for progress",
...
def get_prepare_status():
    ...
    Request (JSON):
        {
            ...
            "simulation_id": "sim_xxxx"      // Optional，Simulation ID（用于检查已完成的准备）
        }
    ...
    返回：
        {
            ...
            "data": {
                ...
                "prepare_info": {...}            // 已Preparation complete时的详细信息
            }
        }
    """
    ...
    try:
        ...
        if not task_id:
            ...
            return jsonify({
                "success": False,
                "error": "Please provide task_id 或 simulation_id"
            }), 400
        ...
        if not task:
            # Task not found，但如果有simulation_id，检查是否已Preparation complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    try:
        ...
        # Sort by creation time descending排序，返回最新的
...
def get_simulation_history():
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get simulation config信息（从 simulation_config.json 读取 simulation_requirement）
        ...
        logger.error(f"获取历史Simulation failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    实时Get simulation的Agent Profile（用于在生成过程中实时查看进度）
...
def get_simulation_config_realtime(simulation_id: str):
    """
    实时Get simulation config（用于在生成过程中实时查看进度）
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation config（LLM智能生成的完整配置）
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": f"Simulation config does not exist, please call /prepare API"
            }), 404
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": "配置文件不存在，请先调用 /prepare API"
            }), 404
...
def download_simulation_script(script_name: str):
    """
    下载Simulation execution脚本文件（通用脚本，位于 backend/scripts/）
...
# ============== Simulation executionControl API ==============
    ...
    Request (JSON):
        {
            ...
            "force": false                         // Optional: 强制重新开始（会停止运行中的模拟并清理Log）
        }

    关于 force 参数：
        - 启用后，如果模拟正在运行或已完成，会先停止并清理运行Log
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"模拟正在运行中，请先调用 /stop API停止，或使用 force=true 强制重新开始"
                            }), 400

                # 如果是强制模式，清理运行Log
                if force:
                    logger.info(f"强制模式：清理模拟Log {simulation_id}")
                ...
                return jsonify({
                    "success": False,
                    "error": f"Simulation not prepared，当前状态: {state.status.value}，请先调用 /prepare API"
                }), 400
        ...
        logger.error(f"启动Simulation failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def stop_simulation():
    ...
    try:
        ...
        logger.error(f"停止Simulation failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_run_status(simulation_id: str):
    """
    获取Simulation execution实时状态（用于前端轮询）
...
def get_run_status_detail(simulation_id: str):
    """
    获取Simulation execution详细状态（包含所有动作）
...
def get_simulation_timeline(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Get timeline失败: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_agent_stats(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Get Agent statistics失败: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def interview_agent():
    ...
    Request (JSON):
        {
            ...
            "prompt": "你对这件事有什么看法？",  // Required，Interview问题
            "platform": "twitter",             // Optional，指定平台（twitter/reddit）
                                               // 不指定时：双平台模拟同时Interview两个平台
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running或已关闭。请确保模拟已完成并进入等待命令模式。"
            }), 400
        
        # 优化prompt，添加prefix避免AgentCall tool
        ...
        return jsonify({
            "success": False,
            "error": f"等待InterviewResponse超时: {str(e)}"
        }), 504
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",       // Required，Simulation ID
            "interviews": [                    // Required，Interview列表
                {
                    "agent_id": 0,
                    "prompt": "你对A有什么看法？",
                    "platform": "twitter"      // Optional，指定该Agent的Interview平台
                },
                {
                    "agent_id": 1,
            ...
            "platform": "reddit",              // Optional，默认平台（被每项的platform覆盖）
                                               // 不指定时：双平台模拟每个Agent同时Interview两个平台
    ...
    try:
        ...
        if not interviews or not isinstance(interviews, list):
            return jsonify({
                "success": False,
                "error": "Please provide interviews（Interview列表）"
            }), 400
        ...
        for i, interview in enumerate(interviews):
            if 'agent_id' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"Interview列表Round{i+1}项缺少 agent_id"
                }), 400
            if 'prompt' not in interview:
                return jsonify({
                    "success": False,
                    "error": f"Interview列表Round{i+1}项缺少 prompt"
                }), 400
            # 验证每项的platform（如果有）
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"Interview列表Round{i+1}项的platform只能是 'twitter' 或 'reddit'"
                }), 400
        ...
        # 优化每个Interview项的prompt，添加prefix避免AgentCall tool
        ...
        return jsonify({
            "success": False,
            "error": f"等待批量InterviewResponse超时: {str(e)}"
        }), 504
...
def interview_all_agents():
    """
    Global interview - 使用相同问题Interview所有Agent
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // Required，Simulation ID
            "prompt": "你对这件事整体有什么看法？",  // Required，Interview问题（所有Agent使用相同问题）
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "error": f"等待全局InterviewResponse超时: {str(e)}"
        }), 504
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",          // Optional，平台类型（reddit/twitter）
                                           // 不指定则返回两个平台的所有历史
            "agent_id": 0,                 // Optional，只获取该Agent的Interview历史

...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is being built, please do not resubmit. To force rebuild, add force: true",
        ...
        logger.info(f"Creating graph build task: task_id={task_id}, project_id={project_id}")
        ...
        def build_task():
            ...
            try:
                ...
                # Wait for Zep processing (check each episode processed status)
                task_manager.update_task(
                    task_id,
        ...
        return jsonify({
            ...
            "data": {
                "project_id": project_id,
                "task_id": task_id,
                "message": "Graph build task started, please check via /task/{task_id} for progress"
            }
        })

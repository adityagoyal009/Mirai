...
- Entities will influence, repost, comment, and respond to each other.
- We need to simulate the reactions of various parties and information dissemination paths in public opinion events.
...
- Representatives of specific groups (e.g., alumni associations, fan clubs, rights defense groups, etc.)
    ...
    "entity_types": [
        {
            ...
            "description": "Short description (English, maximum 100 characters)",
            "attributes": [
                {
...
A. **Fallback types (Must be included, placed as the last 2 in the list)**:
   ...
   - `Organization`: Fallback type for any organization. When an organization does not belong to other more specific organization types, classify them here.
...
- Various characters will appear in the text, such as "primary school teacher", "passerby", "a certain netizen".
...
- Ensure the source_targets of the relationships cover the entity types you defined.
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        # If text exceeds 50K characters, truncate (only affects content sent to LLM, does not affect graph building)
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            combined_text = combined_text[:self.MAX_TEXT_LENGTH_FOR_LLM]
            combined_text += f"\n\n...(Original text is {original_length} characters, first {self.MAX_TEXT_LENGTH_FOR_LLM} characters truncated for ontology analysis)..."
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Final check to not exceed limits (defensive programming)
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        ...
        code_lines = [
            ...
            '# Entity/Edge model definitions for ontology',
            '',
            '',

...
Dynamically updates agent activities from simulation to Zep graph
"""
...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        ...
        Does not add simulation-related prefix to avoid misleading graph updates
        """
        # Generate different descriptions based on action types
        action_descriptions = {
        ...
        # Return "agent name: activity description" format directly, without simulation prefix
...
class ZepGraphMemoryUpdater:
    ...
    Monitors simulation actions log files and updates new agent activities to Zep graph in real-time.
    ...
    # Batch size (how many activities to accumulate per platform before sending)
    BATCH_SIZE = 5
    ...
    def __init__(self, graph_id: str, store=None):
        """
        Initialize the memory updater.

        Args:
            graph_id: Graph ID
            store: EpisodicMemoryStore instance (optional)
        """
        ...
        # Statistics
        self._total_activities = 0  # Actual activities added to queue
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args contains full context information (e.g., original post text, username, etc.).
                ...
                try:
                    ...
                    with self._buffer_lock:
                        ...
                        if len(self._platform_buffers[platform]) >= self.BATCH_SIZE:
                            ...
                            # Send interval, avoid sending too fast
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                self.store.add_episodes(
                    ...
                    metadatas=[{"type": "activity", "platform": platform}],
                )
                ...
                if attempt < self.MAX_RETRIES - 1:
                    ...
                    logger.error(f"Batch send to Zep failed after retrying{self.MAX_RETRIES} times: {e}")
    ...
    def _flush_remaining(self):
        ...
        with self._buffer_lock:
            for platform, buffer in self._platform_buffers.items():
                if buffer:
                    display_name = self._get_platform_display_name(platform)
                    logger.info(f"Sent {display_name}platform remaining  {len(buffer)}   activities")

...
class SimulationIPCClient:
    ...
    def send_command(
        ...
        raise TimeoutError(f"Waiting for command response timed out ({timeout}s)")
    ...
    def send_interview(
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None: In dual-platform simulation, interview on both platforms simultaneously; in single-platform, interview on that platform
            timeout: Timeout
    ...
    def send_batch_interview(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                - "twitter": Default: interview on Twitter platform only
                - "reddit": Default: interview on Reddit platform only
                - None: In dual-platform simulation, interview each agent on both platforms simultaneously
            timeout: Timeout
...
class SimulationIPCServer:
    """
    Simulation IPC Server (used by simulation script)
    
    Polls command directory, executes commands and returns responses
    """

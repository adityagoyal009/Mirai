...
Converts entities from Zep graph to OASIS simulation platform Agent Profile format

优化改进：
1. Calls Zep retrieval to further enrich node info
...
class OasisAgentProfile:
    ...
    # Optional fields - Reddit风格
    karma: int = 1000
    
    # Optional fields - Twitter风格
...
class OasisProfileGenerator:
    ...
    Converts entities from Zep graph to OASIS Agent Profile
    
    优化特性：
    1. Calls Zep graph retrieval for richer context
    2. Generates very detailed personas (including basic info, career, personality, social media behavior, etc.)
    3. Distinguishes between individual entities and abstract group entities
    """
    ...
    # Individual-type entities (need specific personas)
    INDIVIDUAL_ENTITY_TYPES = [
        "student", "alumni", "professor", "person", "publicfigure", 
        "expert", "faculty", "official", "journalist", "activist"
    ]
    
    # Group/institution-type entities (need representative personas)
    GROUP_ENTITY_TYPES = [
    ...
    def __init__(
        ...
        # Memory store for retrieval-enriched context
        self.graph_id = graph_id
        self._memory_store = None
        try:
            from subconscious.memory import EpisodicMemoryStore
            self._memory_store = EpisodicMemoryStore(persist_path=Config.CHROMADB_PERSIST_PATH)
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep graph hybrid search to get rich entity-related info
        
        Zep没有内置混合搜索API，需要分别搜索edges和nodes然后合并结果。
        使用并行请求同时搜索，提高效率。
        ...
        comprehensive_query = f"All information about {entity_name}: activities, events, relationships, background"

        def search_edges():
            """Search edges (facts/relationships) via ChromaDB."""
            try:
                hits = self._memory_store.search(
                    self.graph_id, comprehensive_query, limit=30, collection_suffix="_edges"
                )
                return hits
            except Exception as e:
                logger.debug(f"Edge search failed: {e}")
                return None

        def search_nodes():
            """Search nodes (entity summaries) via ChromaDB."""
            try:
                hits = self._memory_store.search(
                    self.graph_id, comprehensive_query, limit=20, collection_suffix="_nodes"
                )
                return hits
            except Exception as e:
                logger.debug(f"Node search failed: {e}")
                return None
        ...
        try:
            ...
            # Parse edge search results (ChromaDB format: list of dicts)
            all_facts = set()
            if edge_result:
                for hit in edge_result:
                    doc = hit.get("document", "")
                    meta = hit.get("metadata", {})
                    fact = meta.get("fact", doc)
                    if fact:
                        all_facts.add(fact)
            results["facts"] = list(all_facts)

            # Parse node search results (ChromaDB format: list of dicts)
            all_summaries = set()
            if node_result:
                for hit in node_result:
                    meta = hit.get("metadata", {})
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        3. Rich info from Zep hybrid search
        """
        context_parts = []
    ...
    def _generate_profile_with_llm(
        ...
        - Group/institution entity: generate representative account settings
        """
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if finish_reason == 'length':
                    logger.warning(f"LLM output truncated (attempt {attempt+1}), Trying to repair...")
    ...
    def _fix_truncated_json(self, content: str) -> str:
        """Repair truncated JSON (output truncated by max_tokens limit)"""
        import re
        ...
        # 检查是否有未闭合的 characters串
        # 简单检查：如果最后一个引号后没有逗号或Close brackets，可能是 characters串被截断
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 1. First try to repair truncated cases
        content = self._fix_truncated_json(content)
        ...
        if json_match:
            ...
            # 3. 处理 characters串中的换行符问题
            # 找到所有 characters串值并替换其中的换行符
            def fix_string_newlines(match):
                s = match.group(0)
                # 替换 characters串内的实际换行符为空格
                s = s.replace('\n', ' ').replace('\r', ' ')
        ...
        logger.warning(f"JSON repair failed, return basic structure")
        return {
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Retrieved 系统提示词"""
        base_prompt = "You are an expert in generating social media user profiles. Generate detailed, realistic personas for public opinion simulation, maximizing the restoration of existing reality. Must return valid JSON format, all string values cannot contain unescaped newline characters."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate detailed social media user personas for the entity, maximizing the restoration of existing reality.
...
2. persona: Detailed persona description (2000 words of plain text), must include:
   ...
   - Character background (important experiences, connection to the event, social relationships)
   ...
   - Stance and viewpoints (attitude towards the topic, content that might provoke/move them)
   - Unique characteristics (catchphrases, special experiences, personal hobbies)
   - Personal memory (important part of persona, introduce this individual's connection to the event, and their existing actions and reactions in the event)
3. age: Age number (must be an integer)
4. gender: Gender, must be in English: "male" or "female"
5. mbti: MBTI type (e.g., INTJ, ENFP, etc.)
6. country: Country (e.g., "China", "US")
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate detailed social media account settings for the institution/group entity, maximizing the restoration of existing reality.
...
2. persona: Detailed account setting description (2000 words of plain text), must include:
   - Basic institution info (official name, nature of institution, founding background, main functions)
   ...
   - Stance and attitude (official stance on core topics, how to handle controversies)
   - Special notes (profile of the group it represents, operating habits)
   - Institution memory (important part of persona, introduce the institution's connection to the event, and their existing actions and reactions in the event)
3. age: Fixed to 30 (virtual age for institutional accounts)
4. gender: Fixed to "other" (institutional accounts use other to denote non-personal)
5. mbti: MBTI type, used to describe account style, e.g., ISTJ represents rigorous and conservative
6. country: Country (e.g., "China", "US")
7. profession: Description of institution functions
...
- Institution account speeches should match its identity positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: Graph ID，用于Zep检索Retrieved 更丰富上下文
            ...
            output_platform: Output platform format ("reddit" 或 "twitter")
        ...
        # Execute using thread pool in parallel
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    # Write to file in real-time（即使是备用人设）
                    save_profiles_realtime()
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        ...
        # Output to console only (avoid duplicates, logger no longer outputs full content)
        print(output)
    ...
    def save_profiles(
        ...
        Args:
            ...
            file_path: File path
            platform: Platform type ("reddit" 或 "twitter")
        """
        if platform == "twitter":
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        - user_char: Detailed persona描述（注入到LLM系统提示中，指导Agent行为）

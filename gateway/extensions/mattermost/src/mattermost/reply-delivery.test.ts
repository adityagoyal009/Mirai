...
import type { MiraiConfig } from "openclaw/plugin-sdk/mattermost";
...
describe("deliverMattermostReplyPayload", () => {
  it("passes agent-scoped mediaLocalRoots when sending media paths", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mirai-mm-state-"));
    ...
    try {
      ...
      const cfg = {} satisfies MiraiConfig;

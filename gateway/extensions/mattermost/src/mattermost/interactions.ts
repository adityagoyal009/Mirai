...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__mirai_channel_id";
...
type InteractionCallbackConfig = Pick<MiraiConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "mirai-mattermost-interactions").update(botToken).digest("hex");
}

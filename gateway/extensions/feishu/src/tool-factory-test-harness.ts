import type { AnyAgentTool, MiraiPluginApi } from "openclaw/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: MiraiPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<MiraiPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},

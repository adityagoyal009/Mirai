...
import type { MiraiPluginApi, MiraiPluginService } from "openclaw/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: MiraiPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: MiraiPluginApi["config"]): string[] {
...
export default function register(api: MiraiPluginApi) {
  ...
  const timerService: MiraiPluginService = {

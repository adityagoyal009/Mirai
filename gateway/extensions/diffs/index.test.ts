...
import type { MiraiPluginApi } from "openclaw/plugin-sdk/diffs";
...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<MiraiPluginApi["registerHttpRoute"]>[0];
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<MiraiPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as MiraiPluginApi);

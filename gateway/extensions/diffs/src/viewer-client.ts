...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-mirai-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-mirai-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.miraiDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.miraiDiffsError = "true";

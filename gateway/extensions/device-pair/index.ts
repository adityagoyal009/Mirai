...
import type { MiraiPluginApi } from "openclaw/plugin-sdk/device-pair";
import {
...
function resolveGatewayPort(cfg: MiraiPluginApi["config"]): number {
  const envPort =
...
function resolveAuthLabel(cfg: MiraiPluginApi["config"]): ResolveAuthLabelResult {
...
async function resolveGatewayUrl(api: MiraiPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Mirai iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Mirai iOS app:",
            "",
            "```",
            qrAscii,
            "```",

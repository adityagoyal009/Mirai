...
import type { MiraiPluginApi, MiraiPluginToolContext } from "openclaw/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<MiraiPluginApi> = {}): MiraiPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<MiraiPluginToolContext> = {}): MiraiPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mirai-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: MiraiPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }

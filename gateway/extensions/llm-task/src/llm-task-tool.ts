...
// NOTE: This extension is intended to be bundled with Mirai.
// When running from source (tests/dev), Mirai internals live under src/.
// When running from a built install, internals live under dist/ (no src/ tree).
// So we resolve internal imports dynamically with src-first, dist-fallback.
import type { MiraiPluginApi } from "openclaw/plugin-sdk/llm-task";
...
export function createLlmTaskTool(api: MiraiPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via mirai.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredMiraiTmpDir(), "mirai-llm-task-"),
        );

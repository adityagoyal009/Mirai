...
vi.mock("mirai/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("mirai/plugin-sdk/diagnostics-otel")>(
    "mirai/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { MiraiPluginServiceContext } from "openclaw/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "openclaw/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/mirai-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): MiraiPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("mirai.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("mirai.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("mirai.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("mirai.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("mirai.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("mirai.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("mirai.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("mirai.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("mirai.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("mirai.webhook.processed");
    expect(spanNames).toContain("mirai.message.processed");
    expect(spanNames).toContain("mirai.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["mirai.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("mirai.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "mirai.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["mirai.reason"]).toBe("string");
    expect(String(attrs?.["mirai.reason"])).not.toContain(

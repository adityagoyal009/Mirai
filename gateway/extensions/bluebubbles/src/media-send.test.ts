...
import type { MiraiConfig, PluginRuntime } from "openclaw/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): MiraiConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mirai-bb-media-"));
  tempDirs.push(dir);
  return dir;
}

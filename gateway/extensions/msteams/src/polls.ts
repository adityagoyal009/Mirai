...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["miraiPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["mirai", "pollId"]) ??
    readNestedString(value, ["mirai", "poll", "id"]) ??
    readNestedString(value, ["data", "miraiPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "mirai", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "mirai poll vote",
          displayText: "Vote recorded",
          value: { miraiPollId: pollId, pollId },
        },
      },
    ],
  };

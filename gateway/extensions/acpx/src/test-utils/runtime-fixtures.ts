...
import { resolvePreferredMiraiTmpDir } from "../../../../src/infra/tmp-mirai-dir.js";
...
const miraiShell = process.env.OPENCLAW_SHELL || "";
...
async function ensureMockCliScriptPath(): Promise<string> {
  ...
  sharedMockCliScriptPath = (async () => {
    const dir = await mkdtemp(
      path.join(resolvePreferredMiraiTmpDir(), "mirai-acpx-runtime-test-"),
    );
    tempDirs.push(dir);

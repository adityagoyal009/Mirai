...
const lineConfigBase = createScopedChannelConfigBase<ResolvedLineAccount, MiraiConfig>({
  sectionKey: "line",
...
const resolveLineDmPolicy = createScopedDmSecurityResolver<ResolvedLineAccount>({
  ...
  approveHint: "mirai pairing approve line <code>",
...
export const linePlugin: ChannelPlugin<ResolvedLineAccount> = {
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      ...
      await line.pushMessageLine(id, "Mirai: your access has been approved.", {
  ...
  gateway: {
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      const nextCfg = { ...cfg } as MiraiConfig;
      const lineConfig = (cfg.channels?.line ?? {}) as LineConfig;

...
describe("memory index", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mirai-mem-fixtures-"));
  ...
  beforeEach(async () => {
    ...
    vi.stubEnv("MIRAI_TEST_MEMORY_UNSAFE_REINDEX", "1");
    embedBatchCalls = 0;
  ...
  it("reindexes sessions when source config adds sessions to an existing index", async () => {
    ...
    try {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.MIRAI_STATE_DIR = previousStateDir;
      }

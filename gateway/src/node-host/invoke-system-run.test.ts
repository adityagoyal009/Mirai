...
describe("handleSystemRunInvoke mac app exec host routing", () => {
  ...
  async function withTempApprovalsHome<T>(params: {
    ...
    const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "mirai-exec-approvals-"));
    const previousMiraiHome = process.env.MIRAI_HOME;
    process.env.MIRAI_HOME = tempHome;
    saveExecApprovals(params.approvals);
    try {
      return await params.run({ tempHome });
    } finally {
      if (previousMiraiHome === undefined) {
        delete process.env.MIRAI_HOME;
      } else {
        process.env.MIRAI_HOME = previousMiraiHome;
      }
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "mirai-approval-cwd-link-"));
  ...
  it("uses canonical executable path for approval-based relative command execution", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "mirai-approval-cwd-real-"));
  ...
  it("denies ./sh wrapper spoof in allowlist on-miss mode before execution", async () => {
    const marker = path.join(os.tmpdir(), `mirai-wrapper-spoof-${process.pid}-${Date.now()}`);
    const runCommand = vi.fn(async () => {
  ...
  it("denies semicolon-chained shell payloads in allowlist mode without explicit approval", async () => {
    const payloads = ["mirai status; id", "mirai status; cat /etc/passwd"];

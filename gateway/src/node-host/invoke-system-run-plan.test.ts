...
function withFakeRuntimeBin<T>(params: { binName: string; run: () => T }): T {
  return withFakeRuntimeBins({
    ...
    tmpPrefix: `mirai-${params.binName}-bin-`,
    run: params.run,
  });
}
...
function withFakeRuntimeBins<T>(params: {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), params.tmpPrefix ?? "mirai-runtime-bins-"));
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "mirai-ruby-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "mirai-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "mirai-perl-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "mirai-perl-preload-load-path-",
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "mirai-inline-shell-node-",
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  for (const testCase of cases) {
    it.runIf(process.platform !== "win32")(testCase.name, () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "mirai-approval-hardening-"));
      const oldPath = process.env.PATH;
      ...
      withFakeRuntimeBins({
        ...
        run: () => {
          withScriptOperandPlanFixture(
            {
              tmpPrefix: "mirai-approval-script-plan-",
              fixture: runtimeCase,
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "mirai-shell-option-value-"));
    try {

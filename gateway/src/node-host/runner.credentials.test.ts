...
function createRemoteGatewayTokenRefConfig(tokenId: string): MiraiConfig {
  return {
    secrets: {
      providers: {
        default: { source: "env" },
...
describe("resolveNodeHostGatewayCredentials", () => {
  ...
  it("prefers MIRAI_GATEWAY_TOKEN over configured refs", async () => {
    const config = createRemoteGatewayTokenRefConfig("REMOTE_GATEWAY_TOKEN");

    await withEnvAsync(
      {
        MIRAI_GATEWAY_TOKEN: "token-from-env",
        MIRAI_GATEWAY_PASSWORD: undefined,
        REMOTE_GATEWAY_TOKEN: "token-from-ref",
      },
      async () => {

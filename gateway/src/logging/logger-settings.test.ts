...
let originalMiraiLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.MIRAI_TEST_FILE_LOG;
  originalMiraiLogLevel = process.env.MIRAI_LOG_LEVEL;
  delete process.env.MIRAI_TEST_FILE_LOG;
  delete process.env.MIRAI_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.MIRAI_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalMiraiLogLevel === undefined) {
    delete process.env.MIRAI_LOG_LEVEL;
  } else {
    process.env.MIRAI_LOG_LEVEL = originalMiraiLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    process.env.MIRAI_TEST_FILE_LOG = "1";
    const settings = logging.getResolvedLoggerSettings();

...
export const DEFAULT_LOG_DIR = resolvePreferredMiraiTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "mirai.log"); // legacy single-file path
...
function resolveSettings(): ResolvedSettings {
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.MIRAI_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[mirai] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }

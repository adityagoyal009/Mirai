...
export const FIELD_HELP: Record<string, string> = {
  meta: "Metadata fields automatically maintained by Mirai to record write/version history for this config file. Keep these values system-managed and avoid manual edits unless debugging migration history.",
  "meta.lastTouchedVersion": "Auto-set when Mirai writes the config.",
  ...
  "env.vars":
    "Explicit key/value environment variable overrides merged into runtime process environment for Mirai. Use this for deterministic env configuration instead of relying only on shell profile side effects.",
  wizard:
  ...
  "wizard.lastRunVersion":
    "Mirai version recorded at the time of the most recent wizard run on this config. Use this when diagnosing behavior differences across version-to-version setup changes.",
  ...
  update:
    "Update-channel and startup-check behavior for keeping Mirai runtime versions current. Use conservative channels in production and more experimental channels only in controlled environments.",
  ...
  "agents.list[].runtime":
    "Optional runtime descriptor for this agent. Use embedded for default Mirai execution or acp for external ACP harness defaults.",
  "agents.list[].runtime.type":
    'Runtime type for this agent: "embedded" (default Mirai runtime) or "acp" (ACP harness defaults).',
  ...
  "agents.list[].runtime.acp.agent":
    "Optional ACP harness agent id to use for this Mirai agent (for example codex, claude).",
  ...
  "gateway.nodes.allowCommands":
    "Extra node.invoke commands to allow beyond the gateway defaults (array of command strings). Enabling dangerous commands here is a security-sensitive override and is flagged by `mirai security audit`.",
  ...
  "models.providers.*.injectNumCtxForOpenAICompat":
    "Controls whether Mirai injects `options.num_ctx` for Ollama providers configured with the OpenAI-compatible adapter (`openai-completions`). Default is true. Set false only if your proxy/upstream rejects unknown `options` payload fields.",
  ...
  "memory.backend":
    'Selects the global memory engine: "builtin" uses Mirai memory internals, while "qmd" uses the QMD sidecar pipeline. Keep "builtin" unless you intentionally operate QMD.',
  "memory.citations":
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `mirai plugins update` to locate install sources).",
  ...
  "agents.defaults.embeddedPi":
    "Embedded Pi runner hardening controls for how workspace-local Pi settings are trusted and applied in Mirai sessions.",
  ...
  "commands.mcp":
    "Allow /mcp chat command to manage Mirai MCP server config under mcp.servers (default: false).",
  "commands.plugins":
  ...
  mcp: "Global MCP server definitions managed by Mirai. Embedded Pi and other runtime adapters can consume these servers without storing them inside Pi-owned project settings.",
  "mcp.servers":
    "Named MCP server definitions. Mirai stores them in its own config and runtime adapters decide which transports are supported at execution time.",
  session:
  ...
  "session.parentForkMaxTokens":
    "Maximum parent-session token count allowed for thread/session inheritance forking. If the parent exceeds this, Mirai starts a fresh thread session instead of forking; set 0 to disable this protection.",
  "session.mainKey":
  ...
  hooks:
    "Inbound webhook automation surface for mapping external events into wake or agent actions in Mirai. Keep this locked down with explicit token/session/agent controls before exposing it beyond trusted networks.",
  "hooks.enabled":
  ...
  "channels.irc":
    "IRC channel provider configuration and compatibility settings for classic IRC transport workflows. Use this section when bridging legacy chat infrastructure into Mirai.",
  "channels.defaults":
  ...
  "channels.discord.eventQueue.listenerTimeout": `Canonical Discord listener timeout control in ms for gateway normalization/enqueue handlers. Default is ${DISCORD_DEFAULT_LISTENER_TIMEOUT_MS} in Mirai; set per account via channels.discord.accounts.<id>.eventQueue.listenerTimeout.`,

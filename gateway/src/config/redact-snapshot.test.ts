...
import type { ConfigFileSnapshot } from "./types.mirai.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.mirai/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/mirai/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/mirai/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/mirai/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/mirai/nickserv-password.txt");

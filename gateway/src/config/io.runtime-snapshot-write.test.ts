...
function createSourceConfig(): MiraiConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createRuntimeConfig(): MiraiConfig {
  return {
    models: {
      providers: {
        openai: {
...
describe("runtime config snapshot writes", () => {
  it("returns the source snapshot when runtime snapshot is active", async () => {
    await withTempHome("mirai-config-runtime-source-", async () => {
  ...
  it("skips source projection for non-runtime-derived configs", async () => {
    await withTempHome("mirai-config-runtime-projection-shape-", async () => {
      ...
      const independentConfig: MiraiConfig = {
        models: {
          providers: {
            openai: {
              baseUrl: "https://api.openai.com/v1",
  ...
  it("preserves source secret refs when writeConfigFile receives runtime-resolved config", async () => {
    await withTempHome("mirai-config-runtime-write-", async (home) => {
  ...
  it("refreshes the runtime snapshot after writes so follow-up reads see persisted changes", async () => {
    await withTempHome("mirai-config-runtime-write-refresh-", async (home) => {
      const configPath = path.join(home, ".mirai", "mirai.json");
      const sourceConfig: MiraiConfig = {
        models: {
          providers: {
      ...
      const nextRuntimeConfig: MiraiConfig = {
        ...runtimeConfig,
  ...
  it("keeps the last-known-good runtime snapshot active while a specialized refresh is pending", async () => {
    await withTempHome("mirai-config-runtime-refresh-pending-", async (home) => {
      const configPath = path.join(home, ".mirai", "mirai.json");
      const sourceConfig = createSourceConfig();
      const runtimeConfig = createRuntimeConfig();
      const nextRuntimeConfig: MiraiConfig = {

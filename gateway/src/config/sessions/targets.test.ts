...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): MiraiConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: MiraiConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveSessionStoreTargets", () => {
  it("resolves all configured agent stores", () => {
    const cfg: MiraiConfig = {
      session: {
        store: "~/.mirai/agents/{agentId}/sessions/sessions.json",
      },
      agents: {
    ...
    expect(targets).toEqual([
      {
        ...
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".mirai/agents/main/sessions/sessions.json"),
        ),
      },
      {
        agentId: "work",
        storePath: path.resolve(
          path.join(process.env.HOME ?? "", ".mirai/agents/work/sessions/sessions.json"),
        ),
      },
    ]);
  });

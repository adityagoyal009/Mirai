...
This directory contains hooks that ship with Mirai. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.mirai/workspace`)

**Enable**:

```bash
mirai hooks enable session-memory
```
...
```bash
mirai hooks enable bootstrap-extra-files
```
...
**Output**: `~/.mirai/logs/commands.log`

**Enable**:

```bash
mirai hooks enable command-logger
```
...
homepage: https://docs.mirai.ai/automation/hooks#my-hook
metadata:
  { "mirai": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.mirai/hooks/` (shared across workspaces)
...
```bash
mirai hooks info session-memory
```
...
```bash
mirai hooks enable session-memory
mirai hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.mirai/mirai.json`:
...
2. Restart gateway: `pkill -9 -f 'mirai.*gateway' && pnpm mirai gateway`
3. Enable the hook: `mirai hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.mirai.ai/automation/hooks

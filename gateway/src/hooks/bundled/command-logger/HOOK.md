...
homepage: https://docs.mirai.ai/automation/hooks#command-logger
metadata:
  {
    "mirai":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.mirai/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.mirai/logs/commands.log ~/.mirai/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/mirai`:
   ```
   /home/username/.mirai/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.mirai/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.mirai/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.mirai/logs/commands.log | jq .
```

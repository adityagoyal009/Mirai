...
`;
    ...
    expect(parsed.mirai.emoji).toBe("💾");
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.mirai.emoji).toBe("📝");
    expect(parsed.mirai.events).toEqual(["command"]);
    expect(parsed.mirai.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.mirai.install[0].kind).toBe("bundled");
  });
...
describe("resolveMiraiMetadata", () => {
  it("extracts mirai metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveMiraiMetadata(frontmatter);
  ...
  it("returns undefined when mirai key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        mirai: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Mirai" },
            { id: "npm", kind: "npm", package: "@mirai/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@mirai/hook");
  });
...
homepage: https://docs.mirai.ai/automation/hooks#session-memory
metadata:
  {
    "mirai":
      {
...
`;
    ...
    const mirai = resolveMiraiMetadata(frontmatter);
    expect(mirai).toBeDefined();
    expect(mirai?.emoji).toBe("💾");
    expect(mirai?.events).toEqual(["command:new", "command:reset"]);
    expect(mirai?.requires?.config).toEqual(["workspace.dir"]);
    expect(mirai?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const mirai = resolveMiraiMetadata(frontmatter);
    expect(mirai?.emoji).toBe("disk");
    expect(mirai?.events).toEqual(["command:new"]);
  });
});

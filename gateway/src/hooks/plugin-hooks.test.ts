...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "mirai-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.MIRAI_BUNDLED_HOOKS_DIR;
    process.env.MIRAI_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });

  afterEach(() => {
    clearInternalHooks();
    if (previousBundledHooksDir === undefined) {
      delete process.env.MIRAI_BUNDLED_HOOKS_DIR;
    } else {
      process.env.MIRAI_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".mirai", "extensions", "sample-bundle");
    ...
    await fsp.writeFile(
      ...
      [
        ...
        'metadata: {"mirai":{"events":["command:new"]}}',
        "---",
        "",
        "# Bundle hook",
        "",
  ...
  function createConfig(enabled: boolean): MiraiConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("mirai-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Mirai hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".mirai", "extensions", "claude-bundle");

...
import type { MiraiConfig, ConfigFileSnapshot } from "../config/config.js";
...
async function listSandboxBrowserContainers(
  ...
  try {
    const result = await execDockerRawFn(
      ["ps", "-a", "--filter", "label=mirai.sandboxBrowser=1", "--format", "{{.Names}}"],
      { allowFailure: true },
    );
    if (result.code !== 0) {
      return null;
  ...
  try {
    const result = await params.execDockerRawFn(
      [
        "inspect",
        "-f",
        '{{ index .Config.Labels "mirai.configHash" }}\t{{ index .Config.Labels "mirai.browserConfigEpoch" }}',
...
export async function collectSandboxBrowserHashLabelFindings(params?: {
  ...
  if (missingHash.length > 0) {
    findings.push({
      ...
      remediation: `${formatCliCommand("mirai sandbox recreate --browser --all")} (add --force to skip prompt).`,
    });
  }
  ...
  if (staleEpoch.length > 0) {
    findings.push({
      ...
      detail:
        ...
        `Expected mirai.browserConfigEpoch=${SANDBOX_BROWSER_SECURITY_HASH_EPOCH}.`,
      remediation: `${formatCliCommand("mirai sandbox recreate --browser --all")} (add --force to skip prompt).`,
    });
  }
  ...
  if (nonLoopbackPublished.length > 0) {
    findings.push({
      ...
      remediation:
        `${formatCliCommand("mirai sandbox recreate --browser --all")} (add --force to skip prompt), ` +
  ...
  if (pluginDirs.length > 0) {
    ...
    if (enabledExtensionPluginIds.length > 0) {
      ...
      for (const context of contexts) {
        ...
        const broadPolicy = isToolAllowedByPolicies("__mirai_plugin_probe__", policies);
  ...
  if (npmPluginInstalls.length > 0) {
    ...
    if (pluginVersionDrift.length > 0) {
      findings.push({
        ...
        remediation:
          "Run `mirai plugins update --all` (or reinstall affected plugins) to refresh install metadata.",
      });
    }
  }
  ...
  if (npmHookInstalls.length > 0) {
    ...
    if (hookVersionDrift.length > 0) {
      findings.push({
        ...
        remediation:
          "Run `mirai hooks update --all` (or reinstall affected hooks) to refresh install metadata.",
      });
    }
  }
  ...
  const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
    ...
    onReadError: (err) => {
      findings.push({
        ...
        remediation:
          "Check file permissions and plugin layout, then rerun `mirai security audit --deep`.",
      });
    },
  });
  ...
  for (const pluginName of pluginDirs) {
    ...
    if (escapedEntries.length > 0) {
      findings.push({
        ...
        remediation:
          "Update the plugin manifest so all mirai.extensions entries stay inside the plugin directory.",
      });
    }
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        remediation:
          "Review the plugin source code carefully before use. If untrusted, remove the plugin from your Mirai extensions state directory.",
      });
...
export async function collectInstalledSkillsCodeSafetyFindings(params: {
  ...
  for (const workspaceDir of workspaceDirs) {
    ...
    for (const entry of entries) {
      if (entry.skill.source === "mirai-bundled") {
        continue;
      }
      ...
      const summary = await skillScanner.scanDirectoryWithSummary(skillDir).catch((err) => {
        findings.push({
          ...
          remediation:
            "Check file permissions and skill layout, then rerun `mirai security audit --deep`.",
        });
        return null;
      });
      if (!summary) {

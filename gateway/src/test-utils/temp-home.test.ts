...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const previousStateDir = process.env.MIRAI_STATE_DIR;

    const tempHome = await createTempHomeEnv("mirai-temp-home-");
    expect(process.env.HOME).toBe(tempHome.home);
    expect(process.env.USERPROFILE).toBe(tempHome.home);
    expect(process.env.MIRAI_STATE_DIR).toBe(path.join(tempHome.home, ".mirai"));
    ...
    expect(process.env.MIRAI_STATE_DIR).toBe(previousStateDir);

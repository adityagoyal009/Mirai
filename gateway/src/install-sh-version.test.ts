...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "mirai-install-sh-"));
  const cliPath = path.join(root, "mirai");
...
MIRAI_BIN="$FAKE_MIRAI_BIN"
resolve_mirai_version`,
    ],
    {
      cwd: process.cwd(),
      encoding: "utf-8",
      env: {
        ...process.env,
        FAKE_MIRAI_BIN: cliPath,
        MIRAI_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Mirai 2026.3.10 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("Mirai dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("Mirai dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "mirai-install-stdin-"));

...
function resolveSpeechProviderPluginEntries(cfg?: MiraiConfig): SpeechProviderPlugin[] {
  const active = getActivePluginRegistry();
  const registry =
    (active?.speechProviders?.length ?? 0) > 0 || !cfg
      ? active
      : loadMiraiPlugins({ config: cfg });
  return registry?.speechProviders?.map((entry) => entry.provider) ?? [];
}

function buildProviderMaps(cfg?: MiraiConfig): {
...
export function listSpeechProviders(cfg?: MiraiConfig): SpeechProviderPlugin[] {

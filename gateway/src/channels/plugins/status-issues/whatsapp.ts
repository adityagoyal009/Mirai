    ...
    collectIssues: ({ account, accountId, issues }) => {
      ...
      if (!linked) {
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("mirai channels login")} (scan QR on the gateway host).`,
        });
        return;
      }
      ...
      if (running && !connected) {
        issues.push({
          ...
          fix: `Run: ${formatCliCommand("mirai doctor")} (or restart the gateway). If it persists, relink via channels login and check logs.`,
        });
      }
    },
  });

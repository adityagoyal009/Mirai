...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: MiraiConfig, policy: ChannelAccessPolicy) => MiraiConfig;
  resolveAllowlist: (params: { cfg: MiraiConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: MiraiConfig; resolved: TResolved }) => MiraiConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: MiraiConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: MiraiConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: MiraiConfig, policy: ChannelAccessPolicy): MiraiConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: MiraiConfig, policy: ChannelAccessPolicy): MiraiConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: MiraiConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: MiraiConfig; resolved: string[] }) => {
      calls.push("apply");

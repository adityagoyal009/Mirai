...
describe("createStaticReplyToModeResolver", () => {
  it("always returns the configured mode", () => {
    expect(createStaticReplyToModeResolver("off")({ cfg: {} as MiraiConfig })).toBe("off");
    expect(createStaticReplyToModeResolver("all")({ cfg: {} as MiraiConfig })).toBe("all");
  });
});
...
describe("createTopLevelChannelReplyToModeResolver", () => {
  it("reads the top-level channel config", () => {
    ...
    expect(
      resolver({
        cfg: { channels: { discord: { replyToMode: "first" } } } as MiraiConfig,
      }),
    ).toBe("first");
  });
  ...
  it("falls back to off", () => {
    const resolver = createTopLevelChannelReplyToModeResolver("discord");
    expect(resolver({ cfg: {} as MiraiConfig })).toBe("off");
  });
});
...
describe("createScopedAccountReplyToModeResolver", () => {
  ...
  it("passes chatType through", () => {
    ...
    expect(resolver({ cfg: {} as MiraiConfig, chatType: "group" })).toBe("first");

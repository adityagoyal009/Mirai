...
function telegramCfg(): MiraiConfig {
  return { channels: { telegram: { botToken: "tok" } } } as MiraiConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: MiraiConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as MiraiConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as MiraiConfig;
...
function createSignalAccountOverrideCfg(): MiraiConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as MiraiConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as MiraiConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as MiraiConfig,
        accountId: undefined,
        params: {

...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: MiraiConfig | null = null;
  ...
  get(key: string, cfg: MiraiConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: MiraiConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: MiraiConfig): void {

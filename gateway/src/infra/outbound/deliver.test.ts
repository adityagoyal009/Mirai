...
import { resolvePreferredMiraiTmpDir } from "../tmp-mirai-dir.js";
...
const telegramChunkConfig: MiraiConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: MiraiConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes Mirai tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredMiraiTmpDir()]),
      }),
    );
  });

  it("includes Mirai tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes Mirai tmp root in whatsapp mediaLocalRoots", async () => {
  ...
  it("includes Mirai tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: MiraiConfig = { channels: { signal: { mediaMaxMb: 2 } } };

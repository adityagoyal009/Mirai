...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Mirai tmp root", async () => {
      const tmpRoot = resolvePreferredMiraiTmpDir();
      ...
      try {
        ...
        const hostTmpOutsideMirai = path.join(os.tmpdir(), "outside-mirai", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",

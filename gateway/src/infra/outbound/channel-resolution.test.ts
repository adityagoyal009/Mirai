...
const loadMiraiPluginsMock = vi.hoisted(() => vi.fn());
...
vi.mock("../../plugins/loader.js", () => ({
  loadMiraiPlugins: (...args: unknown[]) => loadMiraiPluginsMock(...args),
}));
...
describe("outbound channel resolution", () => {
  beforeEach(() => {
    ...
    loadMiraiPluginsMock.mockReset();
  ...
  it("bootstraps plugins once per registry key and returns the newly loaded plugin", async () => {
    ...
    expect(loadMiraiPluginsMock).toHaveBeenCalledWith({
    ...
    expect(loadMiraiPluginsMock).toHaveBeenLastCalledWith({
      config: { autoEnabled: true },
      workspaceDir: "/tmp/workspace",
      runtimeOptions: {
        allowGatewaySubagentBinding: true,
  ...
  it("retries bootstrap after a transient load failure", async () => {
    getChannelPluginMock.mockReturnValue(undefined);
    loadMiraiPluginsMock.mockImplementationOnce(() => {
    ...
    expect(loadMiraiPluginsMock).toHaveBeenCalledTimes(2);
  });
});

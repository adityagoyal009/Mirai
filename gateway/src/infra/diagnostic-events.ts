...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __miraiDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__miraiDiagnosticEventsState) {
    globalStore.__miraiDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__miraiDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: MiraiConfig): boolean {

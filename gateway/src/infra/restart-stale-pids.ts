 ...
 * returning control to the caller (typically `triggerMiraiRestart` →
...
/**
 * Parse mirai gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("mirai")) {
 ...
 * Returns only PIDs that belong to mirai gateway processes (not the current process).
 */

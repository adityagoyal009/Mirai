...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    const issue = await detectPluginInstallPathIssue({
      ...
      install: {
        source: "path",
        sourcePath: "/tmp/mirai-matrix-missing",
        installPath: "/tmp/mirai-matrix-missing",
      },
    });
    ...
    expect(issue).toEqual({
      kind: "missing-path",
      pluginId: "matrix",
      path: "/tmp/mirai-matrix-missing",
    });
    expect(
      formatPluginInstallPathIssue({
        issue: issue!,
        pluginLabel: "Matrix",
        defaultInstallCommand: "mirai plugins install @mirai/matrix",
        repoInstallCommand: "mirai plugins install ./extensions/matrix",
      }),
    ).toEqual([
      "Matrix is installed from a custom path that no longer exists: /tmp/mirai-matrix-missing",
      'Reinstall with "mirai plugins install @mirai/matrix".',
      'If you are running from a repo checkout, you can also use "mirai plugins install ./extensions/matrix".',
    ]);
  });

  it("uses the second candidate path when the first one is stale", async () => {
    const { issue, pluginPath } = await detectMatrixCustomPathIssue("/tmp/mirai-matrix-missing");
    expect(issue).toEqual({
      kind: "custom-path",
      pluginId: "matrix",
      path: pluginPath,
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<mirai plugins install @mirai/matrix>" when you want to return to the standard Matrix plugin.',
      'If you are intentionally running from a repo checkout, reinstall that checkout explicitly with "<mirai plugins install ./extensions/matrix>" after updates.',
    ]);
  });
});

...
const warningFilterKey = Symbol.for("mirai.warning-filter");
...
describe("warning filter", () => {
  ...
  it("installs once and suppresses known warnings at emit time", async () => {
    ...
    try {
      ...
      emitWarning("Visible warning", { type: "Warning", code: "MIRAI_TEST_WARNING" });
      await new Promise((resolve) => setImmediate(resolve));
      expect(
        seenWarnings.find((warning) => warning.code === "MIRAI_TEST_WARNING"),
      ).toBeDefined();
    } finally {

  ...
  it("includes the cross-platform supervisor hint env vars", () => {
    expect(SUPERVISOR_HINT_ENV_VARS).toEqual(
      expect.arrayContaining([
        "LAUNCH_JOB_LABEL",
        "INVOCATION_ID",
        "MIRAI_WINDOWS_TASK_NAME",
        "MIRAI_SERVICE_MARKER",
        "MIRAI_SERVICE_KIND",
      ]),
    );
  });
...
describe("detectRespawnSupervisor", () => {
  it("detects launchd and systemd only from non-blank platform-specific hints", () => {
    expect(detectRespawnSupervisor({ LAUNCH_JOB_LABEL: " ai.mirai.gateway " }, "darwin")).toBe(
      "launchd",
    );
  ...
  it("detects scheduled-task supervision on Windows from either hint family", () => {
    expect(
      detectRespawnSupervisor({ MIRAI_WINDOWS_TASK_NAME: "Mirai Gateway" }, "win32"),
    ).toBe("schtasks");
    expect(
      detectRespawnSupervisor(
        {
          MIRAI_SERVICE_MARKER: "mirai",
          MIRAI_SERVICE_KIND: "gateway",
        },
        "win32",
      ),
    ).toBe("schtasks");
  ...
  it("ignores service markers on non-Windows platforms and unknown platforms", () => {
    ...
    expect(
      detectRespawnSupervisor({ LAUNCH_JOB_LABEL: "ai.mirai.gateway" }, "freebsd"),
    ).toBeNull();
  });
});

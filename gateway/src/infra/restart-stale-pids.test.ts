...
function createMiraiBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "mirai-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation(() => {
    call += 1;
    if (call === 1) {
      return createMiraiBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses mirai-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "mirai-gateway" },
          { pid: process.pid, cmd: "mirai-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'mirai'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple mirai pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "mirai-gateway" },
          { pid: pid2, cmd: "mirai-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-mirai pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks mirai processes — non-mirai listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty mirai stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty mirai stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + mirai pid in stdout — container-restricted lsof reports partial results
          return createMiraiBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "mirai-gateway" }]),
          stderr: "",
        };
      });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'mirai'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("mirai")
      // inside the mid-loop flush: a non-mirai cmd between two entries must not
      // be pushed, but the following mirai entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-mirai entry first, then mirai entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid mirai entry must still be found.
      const stdout = `p0\ncmirai-gateway\np${stalePid}\ncmirai-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero mirai pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-mirai stdout (line 145)", () => {
    it("treats status 1 + non-mirai stdout as port-free (not an mirai process)", () => {
      // status 1 + non-empty stdout where no mirai pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill mirai pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-mirai output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no mirai pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();

...
import { POSIX_MIRAI_TMP_DIR, resolvePreferredMiraiTmpDir } from "./tmp-mirai-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredMiraiTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `mirai-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredMiraiTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_MIRAI_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredMiraiTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredMiraiTmpDir", () => {
  it("prefers /tmp/mirai when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_MIRAI_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/mirai when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_MIRAI_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/mirai when /tmp/mirai is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/mirai when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/mirai exists but is not writable", () => {
    const accessSync = vi.fn((target: string) => {
      if (target === POSIX_MIRAI_TMP_DIR) {
  ...
  it("falls back when /tmp/mirai is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/mirai is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/mirai is group/other writable", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ mode: 0o40777 })) });
  });

  it("repairs existing /tmp/mirai permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_MIRAI_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_MIRAI_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_MIRAI_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("tightened permissions on temp dir"));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/mirai after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_MIRAI_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
    ...
    expect(resolved).toBe(POSIX_MIRAI_TMP_DIR);
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_MIRAI_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(chmodSync).toHaveBeenCalledWith(POSIX_MIRAI_TMP_DIR, 0o700);
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Mirai temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "mirai");
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredMiraiTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_MIRAI_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback Mirai temp dir/);
  });
});

...
describe("matrix migration snapshots", () => {
  ...
  it("creates a backup marker after writing a pre-migration snapshot", async () => {
    await withTempHome(async (home) => {
      const archivePath = path.join(home, "Backups", "mirai-migrations", "snapshot.tar.gz");
      fs.mkdirSync(path.dirname(archivePath), { recursive: true });
      fs.writeFileSync(path.join(home, ".mirai", "mirai.json"), "{}\n", "utf8");
      fs.writeFileSync(path.join(home, ".mirai", "state.txt"), "state\n", "utf8");
    ...
    await withTempHome(async (home) => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          version: 1,
          createdAt: "2026-03-10T18:00:00.000Z",
          archivePath: path.join(home, "Backups", "mirai-migrations", "missing.tar.gz"),
          trigger: "older-run",
          includeWorkspace: false,
        }),
        "utf8",
    ...
    await withTempHome(
      async (home) => {
        ...
        expect(detection.warnings).toContain(
          "Legacy Matrix encrypted state was detected, but the Matrix plugin helper is unavailable. Install or repair @mirai/matrix so Mirai can inspect the old rust crypto store before upgrading.",
        );
        expect(
          hasActionableMatrixMigration({
            cfg,

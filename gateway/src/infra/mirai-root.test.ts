...
describe("resolveMiraiPackageRoot", () => {
  ...
  it("falls through from a non-mirai moduleUrl candidate to cwd", async () => {
    const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
    const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
    setFile(path.join(wrongPkgRoot, "package.json"), JSON.stringify({ name: "not-mirai" }));
    setFile(path.join(cwdPkgRoot, "package.json"), JSON.stringify({ name: "mirai" }));
    const moduleUrl = pathToFileURL(path.join(wrongPkgRoot, "dist", "index.js")).toString();

    expect(resolveMiraiPackageRootSync({ moduleUrl, cwd: cwdPkgRoot })).toBe(cwdPkgRoot);
    await expect(resolveMiraiPackageRoot({ moduleUrl, cwd: cwdPkgRoot })).resolves.toBe(
      cwdPkgRoot,
    );
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    expect(resolveMiraiPackageRootSync({ moduleUrl: "not-a-file-url", cwd: pkgRoot })).toBe(
      pkgRoot,
    );
    await expect(
      resolveMiraiPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
  ...
  it("falls back from a symlinked argv1 to the node_modules package root", () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "mirai");
    state.realpaths.set(abs(argv1), abs(path.join(project, "versions", "current", "mirai.mjs")));
    const pkgRoot = path.join(project, "node_modules", "mirai");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "mirai" }));

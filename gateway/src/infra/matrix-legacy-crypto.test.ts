...
function writeMatrixPluginFixture(rootDir: string): void {
  ...
  fs.writeFileSync(
    path.join(rootDir, "mirai.plugin.json"),
    JSON.stringify({
      id: "matrix",
      configSchema: {
        type: "object",
...
const matrixHelperEnv = {
  MIRAI_BUNDLED_PLUGINS_DIR: (home: string) => path.join(home, "bundled"),
};
...
describe("matrix legacy encrypted-state migration", () => {
  it("extracts a saved backup key into the new recovery-key path", async () => {
    await withTempHome(
      async (home) => {
        ...
        const stateDir = path.join(home, ".mirai");
    ...
    await withTempHome(async (home) => {
      ...
      expect(detection.warnings).toContain(
        "Legacy Matrix encrypted state detected at " +
          ...
          ', but multiple Matrix accounts are configured and channels.matrix.defaultAccount is not set. Set "channels.matrix.defaultAccount" to the intended target account before rerunning "mirai doctor --fix" or restarting the gateway.',
      );
    });
  });
  ...
  it("warns instead of throwing when a legacy crypto path is a file", async () => {
    await withTempHome(async (home) => {
      ...
      expect(detection.plans).toHaveLength(0);
      expect(detection.warnings).toContain(
        `Legacy Matrix encrypted state path exists but is not a directory: ${path.join(stateDir, "matrix", "crypto")}. Mirai skipped automatic crypto migration for that path.`,
      );
    });
  });
  ...
  it("reports a missing matrix plugin helper once when encrypted-state migration cannot run", async () => {
    await withTempHome(
      ...
      {
        env: {
          MIRAI_BUNDLED_PLUGINS_DIR: (home) => path.join(home, "empty-bundled"),
        },
      },
    );
  });

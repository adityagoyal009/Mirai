...
const resolvePreferredMiraiTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));

vi.mock("node:child_process", () => ({
  spawn: (...args: unknown[]) => spawnMock(...args),
}));
vi.mock("./tmp-mirai-dir.js", () => ({
  resolvePreferredMiraiTmpDir: () => resolvePreferredMiraiTmpDirMock(),
}));
...
afterEach(() => {
  envSnapshot.restore();
  spawnMock.mockReset();
  resolvePreferredMiraiTmpDirMock.mockReset();
  resolvePreferredMiraiTmpDirMock.mockReturnValue(os.tmpdir());
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    const result = relaunchGatewayScheduledTask({ MIRAI_PROFILE: "work" });

    expect(result).toMatchObject({
      ok: true,
      method: "schtasks",
      tried: expect.arrayContaining(['schtasks /Run /TN "Mirai Gateway (work)"']),
    });
    ...
    expect(script).toContain('schtasks /Run /TN "Mirai Gateway (work)" >nul 2>&1');
    expect(script).toContain('del "%~f0" >nul 2>&1');
  });

  it("prefers MIRAI_WINDOWS_TASK_NAME overrides", () => {
    spawnMock.mockImplementation((_file: string, args: string[]) => {
      createdScriptPaths.add(decodeCmdPathArg(args[3]));
      return { unref: vi.fn() };
    });
    ...
    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "Mirai Gateway (custom)" >nul 2>&1');
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    const unref = vi.fn();
    const metacharTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mirai&(restart)-"));
    createdTmpDirs.add(metacharTmpDir);
    resolvePreferredMiraiTmpDirMock.mockReturnValue(metacharTmpDir);
    spawnMock.mockReturnValue({ unref });

    relaunchGatewayScheduledTask({ MIRAI_PROFILE: "work" });

...
describe("resolveEffectiveHomeDir", () => {
  it("prefers MIRAI_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands MIRAI_HOME when set to ~", () => {
    const env = {
      MIRAI_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for MIRAI_HOME", () => {
    const result = resolveRequiredHomeDir(
      { MIRAI_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when MIRAI_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ MIRAI_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { MIRAI_HOME: "/srv/mirai-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/mirai-home")}/x`);
  });

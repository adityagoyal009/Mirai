...
describe("fs pinned write helper", () => {
  it.runIf(process.platform !== "win32")("writes through a pinned parent directory", async () => {
    const root = await tempDirs.make("mirai-fs-pinned-root-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = await tempDirs.make("mirai-fs-pinned-root-");
      const outside = await tempDirs.make("mirai-fs-pinned-outside-");
  ...
  it.runIf(process.platform !== "win32")("accepts streamed input", async () => {
    const root = await tempDirs.make("mirai-fs-pinned-root-");
    const sourcePath = path.join(await tempDirs.make("mirai-fs-pinned-src-"), "source.txt");

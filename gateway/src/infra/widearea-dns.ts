...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.MIRAI_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*mirai-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "mirai";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "mirai");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "mirai-gw");
  ...
  records.push(`_mirai-gw._tcp IN PTR ${instanceLabel}._mirai-gw._tcp`);
  records.push(`${instanceLabel}._mirai-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._mirai-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; mirai-content-hash: ${contentHash}\n${contentBody}`;
}

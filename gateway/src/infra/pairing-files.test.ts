...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/mirai-state", "devices")).toEqual({
      dir: path.join("/tmp/mirai-state", "devices"),
      pendingPath: path.join("/tmp/mirai-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/mirai-state", "devices", "paired.json"),
    });
  });

...
function candidateBinDirs(opts: EnsureMiraiPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `mirai` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "mirai");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.MIRAI_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "mirai"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `mirai` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureMiraiCliOnPath(opts: EnsureMiraiPathOpts = {}) {
  if (isTruthyEnvValue(process.env.MIRAI_PATH_BOOTSTRAPPED)) {
    return;
  }

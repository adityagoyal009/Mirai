...
describe("normalizePackageTagInput", () => {
  const packageNames = ["mirai", "@mirai/plugin"] as const;
  ...
  it("strips known package-name prefixes before returning the tag", () => {
    expect(normalizePackageTagInput("mirai@beta", packageNames)).toBe("beta");
    expect(normalizePackageTagInput("@mirai/plugin@2026.2.24", packageNames)).toBe("2026.2.24");
    expect(normalizePackageTagInput("mirai@   ", packageNames)).toBeNull();
  });

  it("treats exact known package names as an empty tag", () => {
    expect(normalizePackageTagInput("mirai", packageNames)).toBeNull();
    expect(normalizePackageTagInput(" @mirai/plugin ", packageNames)).toBeNull();
  });
  ...
  it("returns trimmed raw values when no package prefix matches", () => {
    ...
    expect(normalizePackageTagInput("miraier@beta", packageNames)).toBe("miraier@beta");
  });
});

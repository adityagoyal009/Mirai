...
describe("system-presence version fallback", () => {
  it("uses MIRAI_SERVICE_VERSION when MIRAI_VERSION is not set", async () => {
  ...
  it("prefers MIRAI_VERSION over MIRAI_SERVICE_VERSION", async () => {
    await withPresenceModule(
      {
  ...
  it("uses npm_package_version when MIRAI_VERSION and MIRAI_SERVICE_VERSION are blank", async () => {
    await withPresenceModule(
      {
        MIRAI_VERSION: " ",

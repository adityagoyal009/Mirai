...
const originalMiraiHome = process.env.MIRAI_HOME;
...
afterEach(() => {
  vi.restoreAllMocks();
  if (originalMiraiHome === undefined) {
    delete process.env.MIRAI_HOME;
  } else {
    process.env.MIRAI_HOME = originalMiraiHome;
  }
...
function createHomeDir(): string {
  ...
  tempDirs.push(dir);
  process.env.MIRAI_HOME = dir;
  return dir;
}

function approvalsFilePath(homeDir: string): string {
  return path.join(homeDir, ".mirai", "exec-approvals.json");
}
...
describe("exec approvals store helpers", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    expect(path.normalize(resolveExecApprovalsPath())).toBe(
      path.normalize(path.join(dir, ".mirai", "exec-approvals.json")),
    );
    expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
      path.normalize(path.join(dir, ".mirai", "exec-approvals.sock")),
    );
  });

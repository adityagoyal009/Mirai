...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.MIRAI_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.MIRAI_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set MIRAI_APNS_TEAM_ID and MIRAI_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.MIRAI_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.MIRAI_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.MIRAI_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set MIRAI_APNS_PRIVATE_KEY_P8 or MIRAI_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading MIRAI_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createMiraiPushMetadata(params: {

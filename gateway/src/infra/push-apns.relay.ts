...
const GATEWAY_DEVICE_ID_HEADER = "x-mirai-gateway-device-id";
const GATEWAY_SIGNATURE_HEADER = "x-mirai-gateway-signature";
const GATEWAY_SIGNED_AT_HEADER = "x-mirai-gateway-signed-at-ms";
...
export function resolveApnsRelayConfigFromEnv(
  ...
  const envBaseUrl = normalizeNonEmptyString(env.MIRAI_APNS_RELAY_BASE_URL);
  ...
  const baseUrlSource = envBaseUrl
    ? "MIRAI_APNS_RELAY_BASE_URL"
    : "gateway.push.apns.relay.baseUrl";
  if (!baseUrl) {
    return {
      ok: false,
      error:
        "APNs relay config missing: set gateway.push.apns.relay.baseUrl or MIRAI_APNS_RELAY_BASE_URL",
    };
  }
  ...
  try {
    ...
    if (parsed.protocol === "http:" && !readAllowHttp(env.MIRAI_APNS_RELAY_ALLOW_HTTP)) {
      throw new Error(
        "http relay URLs require MIRAI_APNS_RELAY_ALLOW_HTTP=true (development only)",
      );
    }
    ...
    return {
      ...
      value: {
        ...
        timeoutMs: normalizeTimeoutMs(
          env.MIRAI_APNS_RELAY_TIMEOUT_MS ?? configuredRelay?.timeoutMs,
        ),
      },
    };
  } catch (err) {

...
function resolveVoiceTtsConfig(params: { cfg: MiraiConfig; override?: TtsConfig }): {
  cfg: MiraiConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredMiraiTmpDir(), "discord-voice-"));

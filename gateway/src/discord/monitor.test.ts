...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of Mirai")).toBe("friends-of-mirai");
  ...
  it("matches ids by default and names only when enabled", () => {
    ...
    expect(allowListMatches(allow, { name: "friends-of-mirai" })).toBe(false);
    expect(allowListMatches(allow, { name: "steipete" }, { allowNameMatching: true })).toBe(true);
    expect(
      allowListMatches(allow, { name: "friends-of-mirai" }, { allowNameMatching: true }),
    ).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-mirai" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Mirai"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-mirai");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-mirai": { slug: "friends-of-mirai" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Mirai"),
      guildEntries,
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["mirai-dm"],
        channelId: "1",
        channelName: "Mirai DM",
        channelSlug: "mirai-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({

...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `mirai-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      MIRAI_LAUNCHD_LABEL: `ai.mirai.launchd-int-${testId}`,
      MIRAI_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });

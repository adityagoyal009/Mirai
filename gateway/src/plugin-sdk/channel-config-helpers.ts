...
export function createScopedAccountConfigAccessors<
  ResolvedAccount,
  Config extends MiraiConfig = MiraiConfig,
>(params: {
...
> {
  const base = {
    resolveAllowFrom: ({ cfg, accountId }: { cfg: MiraiConfig; accountId?: string | null }) =>
      mapAllowFromEntries(
...
function setTopLevelChannelEnabledInConfigSection<Config extends MiraiConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends MiraiConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends MiraiConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {

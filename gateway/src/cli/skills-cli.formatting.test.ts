...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "mirai-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "mirai-bundled-skills-test-"));
    process.env.MIRAI_BUNDLED_SKILLS_DIR = tempBundledDir;
  });

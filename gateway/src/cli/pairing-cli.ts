...
export function registerPairingCli(program: Command) {
  ...
  const pairing = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/pairing", "github.com/adityagoyal009/Mirai/tree/main/gateway/docs/cli/pairing")}\n`,
    );
  ...
  pairing
    ...
    .action(async (codeOrChannel, code, opts) => {
      ...
      if (!channelRaw || !resolvedCode) {
        throw new Error(
          `Usage: ${formatCliCommand("mirai pairing approve <channel> <code>")} (or: ${formatCliCommand("mirai pairing approve --channel <channel> <code>")})`,
        );
      }
      if (opts.channel && code != null) {
        throw new Error(
          `Too many arguments. Use: ${formatCliCommand("mirai pairing approve --channel <channel> <code>")}`,
        );
      }
      const channel = parseChannel(channelRaw, channels);
      const accountId = String(opts.account ?? "").trim();

...
export function registerBackupCommand(program: Command) {
  const backup = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/backup", "github.com/adityagoyal009/Mirai/tree/main/gateway/docs/cli/backup")}\n`,
    );
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mirai backup create", "Create a timestamped backup in the current directory."],
          [
            "mirai backup create --output ~/Backups",
            "Write the archive into an existing backup directory.",
          ],
          [
            "mirai backup create --dry-run --json",
            "Preview the archive plan without writing any files.",
          ],
          [
            "mirai backup create --verify",
            "Create the archive and immediately validate its manifest and payload layout.",
          ],
          [
            "mirai backup create --no-include-workspace",
            "Back up state/config without agent workspace files.",
          ],
          ["mirai backup create --only-config", "Back up only the active JSON config file."],
        ])}`,
    )
    .action(async (opts) => {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ...
          [
            "mirai backup verify ~/Backups/latest.tar.gz --json",

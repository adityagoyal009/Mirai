...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.mirai${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.MIRAI_PROFILE = profile;

  const stateDir = env.MIRAI_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.MIRAI_STATE_DIR?.trim()) {
    env.MIRAI_STATE_DIR = stateDir;
  }

  if (!env.MIRAI_CONFIG_PATH?.trim()) {
    env.MIRAI_CONFIG_PATH = path.join(stateDir, "mirai.json");
  }

  if (profile === "dev" && !env.MIRAI_GATEWAY_PORT?.trim()) {
    env.MIRAI_GATEWAY_PORT = "19001";
  }
}

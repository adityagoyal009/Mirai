...
const SANDBOX_EXAMPLES = {
  main: [
    ["mirai sandbox list", "List all sandbox containers."],
    ["mirai sandbox list --browser", "List only browser containers."],
    ["mirai sandbox recreate --all", "Recreate all containers."],
    ["mirai sandbox recreate --session main", "Recreate a specific session."],
    ["mirai sandbox recreate --agent mybot", "Recreate agent containers."],
    ["mirai sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["mirai sandbox list", "List all sandbox containers."],
    ["mirai sandbox list --browser", "List only browser containers."],
    ["mirai sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["mirai sandbox recreate --all", "Recreate all containers."],
    ["mirai sandbox recreate --session main", "Recreate a specific session."],
    ["mirai sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["mirai sandbox recreate --browser --all", "Recreate only browser containers."],
    ["mirai sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["mirai sandbox explain", "Show effective sandbox config."],
    ["mirai sandbox explain --session agent:main:main", "Explain a specific session."],
    ["mirai sandbox explain --agent work", "Explain an agent sandbox."],
    ["mirai sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "github.com/adityagoyal009/Mirai/tree/main/gateway/docs/cli/sandbox")}\n`,
    )
    .action(() => {

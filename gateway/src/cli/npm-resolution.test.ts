...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@mirai/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@mirai/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@mirai/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@mirai/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @mirai/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@mirai/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@mirai/plugin-alpha@1.2.3",
        installPath: "/tmp/mirai/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@mirai/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @mirai/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@mirai/plugin-alpha@latest",
      true,
      "/tmp/mirai/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@mirai/plugin-alpha@latest",
      installPath: "/tmp/mirai/extensions/alpha",
      version: "1.2.3",

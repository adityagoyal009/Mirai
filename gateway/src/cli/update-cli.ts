...
export function registerUpdateCli(program: Command) {
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["mirai update", "Update a source checkout (git)"],
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.mirai.ai/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mirai update status", "Show channel + version status."],
          ["mirai update status --json", "JSON output."],
          ["mirai update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(

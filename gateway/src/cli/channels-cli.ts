    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mirai channels list", "List configured channels and auth profiles."],
          ["mirai channels status --probe", "Run channel status checks and probes."],
          [
            "mirai channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["mirai channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "github.com/adityagoyal009/Mirai/tree/main/gateway/docs/cli/channels",
        )}\n`,
    );

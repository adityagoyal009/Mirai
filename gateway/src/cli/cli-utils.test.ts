...
describe("shouldSkipRespawnForArgv", () => {
  ...
  it("keeps respawn path for normal commands", () => {
    expect(shouldSkipRespawnForArgv(["node", "mirai", "status"])).toBe(false);
  });
});
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "mirai.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("mirai.internal");
    } finally {
      log.mockRestore();
    }
  });

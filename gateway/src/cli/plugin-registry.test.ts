...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("loads only configured channel plugins for configured-channels scope", async () => {
    ...
    expect(mocks.loadMiraiPlugins).toHaveBeenCalledWith(
  ...
  it("reloads when escalating from configured-channels to channels", async () => {
    ...
    expect(mocks.loadMiraiPlugins).toHaveBeenCalledTimes(2);
    expect(mocks.loadMiraiPlugins).toHaveBeenNthCalledWith(
      1,

...
async function restartGatewayWithoutServiceManager(port: number) {
  ...
  if (pids.length > 1) {
    throw new Error(
      `multiple gateway processes are listening on port ${port}: ${formatGatewayPidList(pids)}; use "mirai gateway status --deep" before retrying restart`,
    );
  }
    ...
    postRestartCheck: async ({ warnings, fail, stdout }) => {
      if (restartedWithoutServiceManager) {
        ...
        fail(`Gateway restart timed out after ${restartWaitSeconds}s waiting for health checks.`, [
          formatCliCommand("mirai gateway status --deep"),
          formatCliCommand("mirai doctor"),
        ]);
      }

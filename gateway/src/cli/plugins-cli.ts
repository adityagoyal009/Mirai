...
function applySlotSelectionForPlugin(
  config: MiraiConfig,
  pluginId: string,
): { config: MiraiConfig; warnings: string[] } {
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.mirai.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      let next: MiraiConfig = enableResult.config;
    ...
    .action(async (raw: string, opts: { link?: boolean; pin?: boolean }) => {
      ...
      if (fs.existsSync(resolved)) {
        if (opts.link) {
          ...
          let next: MiraiConfig = enablePluginInConfig(
            {
              ...cfg,
              plugins: {
                ...cfg.plugins,
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.mirai.ai/plugin");
      lines.push("");

...
const CONFIG_SET_EXAMPLE_VALUE = formatCliCommand(
  "mirai config set gateway.port 19001 --strict-json",
);
const CONFIG_SET_EXAMPLE_REF = formatCliCommand(
  "mirai config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN",
);
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "mirai config set secrets.providers.vault --provider-source file --provider-path /etc/openclaw/secrets.json --provider-mode json",
);
const CONFIG_SET_EXAMPLE_BATCH = formatCliCommand(
  "mirai config set --batch-file ./config-set.batch.json --dry-run",
);
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("mirai doctor")}\` ${message}`;
}
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "github.com/adityagoyal009/Mirai/tree/main/gateway/docs/cli/config")}\n`,
    )
    .option(
      "--section <section>",

...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "github.com/adityagoyal009/Mirai/tree/main/gateway/docs/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Mirai hooks")
    ...
    .option("--hook-url <url>", "Mirai hook URL")
    .option("--hook-token <token>", "Mirai hook token")

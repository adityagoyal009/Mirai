...
describe("tryRouteCli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    originalDisableRouteFirst = process.env.MIRAI_DISABLE_ROUTE_FIRST;
    delete process.env.MIRAI_DISABLE_ROUTE_FIRST;
    vi.resetModules();
  ...
  afterEach(() => {
    if (originalDisableRouteFirst === undefined) {
      ...
      process.env.MIRAI_DISABLE_ROUTE_FIRST = originalDisableRouteFirst;
    }
  });

  it("passes suppressDoctorStdout=true for routed --json commands", async () => {
    await expect(tryRouteCli(["node", "mirai", "status", "--json"])).resolves.toBe(true);
  ...
  it("does not pass suppressDoctorStdout for routed non-json commands", async () => {
    await expect(tryRouteCli(["node", "mirai", "status"])).resolves.toBe(true);
  ...
  it("routes status when root options precede the command", async () => {
    await expect(tryRouteCli(["node", "mirai", "--log-level", "debug", "status"])).resolves.toBe(
      true,
    );

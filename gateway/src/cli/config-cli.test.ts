...
import type { ConfigFileSnapshot, MiraiConfig } from "../config/types.js";

/**
 * Test for issue #6070:
 * `mirai config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: MiraiConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: MiraiConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: MiraiConfig, config: MiraiConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: MiraiConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {

...
export async function finalizeSetupWizard(
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.mirai/config.json (gateway.auth.token) or OPENCLAW_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("miraiconfig get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("mirai doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("mirai dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control Mirai."
            : "Copy/paste this URL in a browser on this machine to control Mirai.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("mirai dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://github.com/adityagoyal009/Mirai/tree/main/gateway/docs/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://github.com/adityagoyal009/Mirai/tree/main/gateway/docs/security",
    "Security",
  );
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("mirai configure --section web")}`,
          "",
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("mirai configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://github.com/adityagoyal009/Mirai/tree/main/gateway/docs/tools/web"}`,
          "Docs: https://github.com/adityagoyal009/Mirai/tree/main/gateway/docs/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("mirai configure --section web")}`,
          "",
          "Docs: https://github.com/adityagoyal009/Mirai/tree/main/gateway/docs/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://github.com/adityagoyal009/Mirai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control Mirai."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control Mirai.",
  );

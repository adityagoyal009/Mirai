...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Must read: https://docs.mirai.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runOnboardingWizard(
  ...
  await prompter.intro("Mirai onboarding");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: MiraiConfig = snapshot.valid ? snapshot.config : {};
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("mirai doctor")}\` to repair it, then re-run onboarding.`,
    );
    runtime.exit(1);
    return;
  }

...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Mirai is a hobby project and still in beta. Expect sharp edges.",
      "By default, Mirai is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "Mirai is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run Mirai.",
      ...
      "",
      "Run regularly:",
      "mirai security audit --deep",
      "mirai security audit --fix",
      "",
      "Must read: https://github.com/adityagoyal009/Mirai/tree/main/gateway/docs/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runSetupWizard(
  ...
  await prompter.intro("Mirai setup");
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("mirai doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("mirai doctor")}`,
        `Inspect: ${formatCliCommand("mirai plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("mirai configure")}.`;
  ...
  // ── Multi-model Council/Swarm setup ──────────────────────────
  ...
  while (addMore) {
    const extraAuthChoice = await promptAuthChoiceGrouped({
      prompter,
      store: authStore,
      includeSkip: false,
      config: nextConfig,
    ...
    if (extraAuthChoice === "custom-api-key") {
      const customResult = await promptCustomApiConfig({
        prompter,
        runtime,
        config: nextConfig,
      ...
      const extraAuthResult = await applyAuthChoice({
        authChoice: extraAuthChoice,
        config: nextConfig,
        prompter,
        runtime,
      ...
      nextConfig = extraAuthResult.config;
    }

    const extraModelSelection = await promptDefaultModel({
      config: nextConfig,
      prompter,
      allowKeep: false,
      ignoreAllowlist: true,
    ...
    if (extraModelSelection.config) {
      nextConfig = extraModelSelection.config;
    }
    if (extraModelSelection.model) {
      const modelId = extraModelSelection.model;
      councilModels.push({
        provider: modelId.split("/")[0] ?? "default",
        model: modelId,
        label: modelId,
      });
    }
  ...
  if (councilModels.length > 1) {
    nextConfig = {
      ...
      models: {
        ...nextConfig.models,
        council: { models: councilModels },
      },
    };
    await prompter.note(
      councilModels.map((m) => `- ${m.label}`).join("\n"),
      `Council: ${councilModels.length} models configured`,
    );
  }

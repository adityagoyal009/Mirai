...
const resolveExistingKey = vi.hoisted(() =>
  vi.fn<(config: MiraiConfig, provider: string) => string | undefined>(() => undefined),
);
const hasExistingKey = vi.hoisted(() =>
  vi.fn<(config: MiraiConfig, provider: string) => boolean>(() => false),
);
...
const listConfiguredWebSearchProviders = vi.hoisted(() =>
  vi.fn<(params?: { config?: MiraiConfig }) => PluginWebSearchProviderEntry[]>(() => []),
);
...
describe("finalizeSetupWizard", () => {
  ...
  it("resolves gateway password SecretRef for probe and TUI", async () => {
    ...
    process.env.MIRAI_GATEWAY_PASSWORD = "resolved-gateway-password"; // pragma: allowlist secret

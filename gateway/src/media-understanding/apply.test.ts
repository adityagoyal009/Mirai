...
const TEMP_MEDIA_PREFIX = "mirai-media-";
...
function createGroqAudioConfig(): MiraiConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): MiraiConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): MiraiConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: MiraiConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredMiraiTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        MIRAI_AGENT_DIR: isolatedAgentDir,

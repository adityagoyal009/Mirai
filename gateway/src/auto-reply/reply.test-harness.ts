...
type HomeEnvSnapshot = {
  ...
  MIRAI_STATE_DIR: string | undefined;
  MIRAI_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    MIRAI_AGENT_DIR: process.env.MIRAI_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".mirai", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.MIRAI_STATE_DIR = path.join(home, ".mirai");
    process.env.MIRAI_AGENT_DIR = path.join(home, ".mirai", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".mirai", "agent");

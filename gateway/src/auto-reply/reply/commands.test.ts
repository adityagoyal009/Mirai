...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "mirai-commands-"));
...
function buildParams(commandBody: string, cfg: MiraiConfig, ctxOverrides?: Partial<MsgContext>) {
...
describe("/compact command", () => {
  ...
  it("routes manual compaction with explicit trigger and context metadata", async () => {
    const cfg = {
      commands: { text: true },
      channels: { whatsapp: { allowFrom: ["*"] } },
      session: { store: "/tmp/mirai-session-store.json" },
    } as MiraiConfig;
    const params = buildParams("/compact: focus on decisions", cfg, {
      From: "+15550001",
      To: "+15550002",
    });
    const agentDir = "/tmp/mirai-agent-compact";
...
describe("handleCommands /allowlist", () => {
  ...
  it("removes DM allowlist entries from canonical allowFrom and deletes legacy dm.allowFrom", async () => {
    ...
    for (const testCase of cases) {
      ...
      const written = writeConfigFileMock.mock.calls.at(-1)?.[0] as MiraiConfig;

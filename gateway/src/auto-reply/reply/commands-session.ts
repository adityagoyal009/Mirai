...
import { scheduleGatewaySigusr1Restart, triggerMiraiRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Mirai in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerMiraiRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Mirai via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};

...
const workspaceHarness = createCommandWorkspaceHarness("mirai-command-plugins-");

async function createClaudeBundlePlugin(params: { workspaceDir: string; pluginId: string }) {
  const pluginDir = path.join(params.workspaceDir, ".mirai", "extensions", params.pluginId);
...
describe("handleCommands /plugins", () => {
  ...
  it("lists discovered plugins and inspects plugin details", async () => {
    await withTempHome("mirai-command-plugins-home-", async () => {

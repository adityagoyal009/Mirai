...
function listAgents(cfg: MiraiConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: MiraiConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: MiraiConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<MiraiConfig, EvaluatedBindingsCache>();

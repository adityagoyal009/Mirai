...
describe("provider discovery contract", () => {
  beforeAll(async () => {
    vi.doMock("mirai-gateway/plugin-sdk/agent-runtime", async () => {
    ...
    vi.doMock("mirai-gateway/plugin-sdk/provider-auth", async () => {
      const actual = await vi.importActual<object>("mirai-gateway/plugin-sdk/provider-auth");
      return {
        ...actual,
        ensureAuthProfileStore: ensureAuthProfileStoreMock,
        listProfilesForProvider: listProfilesForProviderMock,
    ...
    vi.doMock("mirai-gateway/plugin-sdk/provider-setup", async () => {
      const actual = await vi.importActual<object>("mirai-gateway/plugin-sdk/provider-setup");
      return {
        ...actual,
    ...
    vi.doMock("mirai-gateway/plugin-sdk/self-hosted-provider-setup", async () => {
      const actual = await vi.importActual<object>(
        "mirai-gateway/plugin-sdk/self-hosted-provider-setup",
      );
      return {
        ...actual,
        buildVllmProvider: (...args: unknown[]) => buildVllmProviderMock(...args),
    ...
    vi.doMock("mirai-gateway/plugin-sdk/ollama-setup", async () => {
      const actual = await vi.importActual<object>("mirai-gateway/plugin-sdk/ollama-setup");
      return {
        ...actual,
        buildOllamaProvider: (...args: unknown[]) => buildOllamaProviderMock(...args),
      };

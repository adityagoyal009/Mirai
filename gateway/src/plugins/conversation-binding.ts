...
const APPROVALS_PATH = "~/.mirai/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "mirai-app-server:thread:",
  "mirai-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("mirai.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("mirai.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Mirai instead.${buildDetachHintSuffix(binding.detachHint)}`;
}

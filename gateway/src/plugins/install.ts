...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing openclaw.extensions; update the plugin package to include openclaw.extensions (for example ["./dist/index.js"]). See https://github.com/adityagoyal009/Mirai/tree/main/gateway/docs/help/troubleshooting#plugin-install-fails-with-missing-openclaw-extensions';
...
async function installBundleFromSourceDir(
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Bundle "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "mirai security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Bundle "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "mirai security audit --deep" after install.`,
    );
  }
...
async function installPluginFromPackageDir(
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "mirai security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "mirai security audit --deep" after install.`,
    );
  }

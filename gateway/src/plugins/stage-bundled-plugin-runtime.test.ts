...
import { discoverMiraiPlugins } from "./discovery.js";
...
describe("stageBundledPluginRuntime", () => {
  it("stages bundled dist plugins as runtime wrappers and links staged dist node_modules", () => {
    const repoRoot = makeRepoRoot("mirai-stage-bundled-runtime-");
  ...
  it("writes wrappers that forward plugin entry imports into canonical dist files", async () => {
    const repoRoot = makeRepoRoot("mirai-stage-bundled-runtime-chunks-");
  ...
  it("keeps plugin command registration on the canonical dist graph when loaded from dist-runtime", async () => {
    const repoRoot = makeRepoRoot("mirai-stage-bundled-runtime-commands-");
    ...
    fs.writeFileSync(
      ...
      [
        "const registry = globalThis.__miraiTestPluginCommands ??= new Map();",
  ...
  it("copies package metadata files but symlinks other non-js plugin artifacts into the runtime overlay", () => {
    const repoRoot = makeRepoRoot("mirai-stage-bundled-runtime-assets-");
    const distPluginDir = path.join(repoRoot, "dist", "extensions", "diffs");
    fs.mkdirSync(path.join(distPluginDir, "assets"), { recursive: true });
    fs.writeFileSync(
      path.join(distPluginDir, "package.json"),
      JSON.stringify(
        { name: "@mirai/diffs", mirai: { extensions: ["./index.js"] } },
        null,
        2,
      ),
      "utf8",
    );
    fs.writeFileSync(path.join(distPluginDir, "mirai.plugin.json"), "{}\n", "utf8");
  ...
  it("preserves package metadata needed for bundled plugin discovery from dist-runtime", () => {
    const repoRoot = makeRepoRoot("mirai-stage-bundled-runtime-discovery-");
    const distPluginDir = path.join(repoRoot, "dist", "extensions", "demo");
    ...
    fs.writeFileSync(
      path.join(distPluginDir, "mirai.plugin.json"),
      JSON.stringify(
        {
          id: "demo",
          channels: ["demo"],
    ...
    const env = {
      ...process.env,
      MIRAI_BUNDLED_PLUGINS_DIR: runtimeExtensionsDir,
    };
    const discovery = discoverMiraiPlugins({
      env,
      cache: false,
    });
  ...
  it("removes stale runtime plugin directories that are no longer in dist", () => {
    const repoRoot = makeRepoRoot("mirai-stage-bundled-runtime-stale-");
  ...
  it("removes dist-runtime when the built bundled plugin tree is absent", () => {
    const repoRoot = makeRepoRoot("mirai-stage-bundled-runtime-missing-");

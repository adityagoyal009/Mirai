...
const fixtureRoot = path.join(os.tmpdir(), `mirai-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.MIRAI_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "mirai.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.MIRAI_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadMiraiPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
function expectTelegramLoaded(registry: ReturnType<typeof loadMiraiPlugins>) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.MIRAI_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.MIRAI_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadMiraiPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const enabledRegistry = loadMiraiPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.MIRAI_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ MIRAI_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {

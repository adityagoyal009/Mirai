...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Mirai uses this to expose
 ...
 *
 * Runs after Mirai resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Mirai auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Mirai wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Mirai builds its generic
...
/**
 * Provider-owned stream wrapper hook after Mirai applies its generic
 ...
 *
 * Runs only after Mirai exhausts normal env/profile/config auth resolution
 ...
 *
 * Runs after Mirai loads the discovered model catalog and merges configured
...
export type ProviderPlugin = {
   ...
   *
   * Mirai calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic Mirai wrappers.
   *
   ...
   *
   * Called after Mirai resolves the raw configured credential but before the
   ...
   * `errorMessage` when Mirai should surface a provider-specific hint for
   ...
   * merging. Mirai deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * Mirai uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `mirai doctor`.
   *
   ...
   *
   * Mirai calls this before falling back to the shared `pi-ai` OAuth

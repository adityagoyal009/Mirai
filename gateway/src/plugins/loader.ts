...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: MiraiPluginDefinition;
  register?: MiraiPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as MiraiPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as MiraiPluginDefinition;
...
export function loadMiraiPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "mirai/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "mirai/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: MiraiPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as MiraiPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,

...
export async function runProviderPluginAuthMethod(params: {
  ...
  opts?: Partial<ProviderAuthOptionBag>;
}): Promise<{ config: MiraiConfig; defaultModel?: string }> {
...
export async function applyAuthChoicePluginProvider(
  ...
  const agentDir =
    params.agentDir ??
    (agentId === defaultAgentId ? resolveMiraiAgentDir() : resolveAgentDir(nextConfig, agentId));
  const workspaceDir =

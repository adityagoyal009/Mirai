...
function makeTempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mirai-plugin-loader-"));
  tempRoots.push(dir);
  return dir;
}
...
describe("plugin loader git path regression", () => {
  it("loads git-style package extension entries when they import plugin-sdk infra-runtime (#49806)", async () => {
    ...
    fs.writeFileSync(
      ...
      `import { resolveOutboundSendDep } from "mirai-gateway/plugin-sdk/infra-runtime";
...
`,
    ...
    const script = `
      ...
      const withAlias = createJiti(${JSON.stringify(jitiBaseFile)}, {
        ...
        extensions: [".ts", ".tsx", ".mts", ".cts", ".mtsx", ".ctsx", ".js", ".mjs", ".cjs", ".json"],
        alias: {
          "mirai-gateway/plugin-sdk/infra-runtime": ${JSON.stringify(copiedChannelRuntimeShim)},
        },
      });

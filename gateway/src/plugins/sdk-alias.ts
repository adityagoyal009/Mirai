...
function hasTrustedMiraiRootIndicator(params: {
  packageRoot: string;
  ...
  const hasMiraiBin =
    (typeof params.packageJson.bin === "string" &&
      params.packageJson.bin.toLowerCase().includes("mirai")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.mirai === "string");
  const hasMiraiEntrypoint = fs.existsSync(path.join(params.packageRoot, "mirai.mjs"));
  return hasCliEntryExport || hasMiraiBin || hasMiraiEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedMiraiRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveMiraiPackageRootSync({ cwd });
  const fromExplicitHints =
    params.argv1 || params.moduleUrl
      ? resolveMiraiPackageRootSync({
          cwd,
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths(params)) {
    ...
    if (resolved) {
      aliasMap[`mirai/plugin-sdk/${subpath}`] = resolved;
    }
  }
  return aliasMap;
}

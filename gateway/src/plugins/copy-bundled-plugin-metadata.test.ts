...
describe("copyBundledPluginMetadata", () => {
  it("copies plugin manifests, package metadata, and local skill directories", () => {
    const repoRoot = makeRepoRoot("mirai-bundled-plugin-meta-");
    ...
    writeJson(path.join(pluginDir, "mirai.plugin.json"), {
      id: "acpx",
    ...
    writeJson(path.join(pluginDir, "package.json"), {
      name: "@mirai/acpx",
      mirai: { extensions: ["./index.ts"] },
    });

    copyBundledPluginMetadata({ repoRoot });

    expect(
      fs.existsSync(path.join(repoRoot, "dist", "extensions", "acpx", "mirai.plugin.json")),
    ).toBe(true);
    expect(
      fs.readFileSync(
    ...
    const bundledManifest = JSON.parse(
      fs.readFileSync(
        path.join(repoRoot, "dist", "extensions", "acpx", "mirai.plugin.json"),
        "utf8",
      ),
    ) as { skills?: string[] };
    ...
    ) as { mirai?: { extensions?: string[] } };
    expect(packageJson.mirai?.extensions).toEqual(["./index.js"]);
  });

  it("relocates node_modules-backed skill paths into bundled-skills and rewrites the manifest", () => {
    const repoRoot = makeRepoRoot("mirai-bundled-plugin-node-modules-");
    ...
    const bundledManifest = JSON.parse(
      fs.readFileSync(
        path.join(repoRoot, "dist", "extensions", "tlon", "mirai.plugin.json"),
        "utf8",
      ),
    ) as { skills?: string[] };
  ...
  it("falls back to repo-root hoisted node_modules skill paths", () => {
    const repoRoot = makeRepoRoot("mirai-bundled-plugin-hoisted-skill-");
    const pluginDir = path.join(repoRoot, "extensions", "tlon");
  ...
  it("omits missing declared skill paths and removes stale generated outputs", () => {
    const repoRoot = makeRepoRoot("mirai-bundled-plugin-missing-skill-");
    const pluginDir = path.join(repoRoot, "extensions", "tlon");
    fs.mkdirSync(pluginDir, { recursive: true });
    writeJson(path.join(pluginDir, "mirai.plugin.json"), {
      id: "tlon",
  ...
  it("retries transient skill copy races from concurrent runtime postbuilds", () => {
    const repoRoot = makeRepoRoot("mirai-bundled-plugin-retry-");
  ...
  it("removes generated outputs for plugins no longer present in source", () => {
    const repoRoot = makeRepoRoot("mirai-bundled-plugin-removed-");
    const staleBundledSkillDir = path.join(
      repoRoot,
      "dist",
      "extensions",
  ...
  it("removes stale dist outputs when a source extension directory no longer has a manifest", () => {
    const repoRoot = makeRepoRoot("mirai-bundled-plugin-manifestless-source-");
    ...
    writeJson(path.join(staleDistDir, "mirai.plugin.json"), {
      id: "google-gemini-cli-auth",
      configSchema: { type: "object" },
    });
    writeJson(path.join(staleDistDir, "package.json"), {
      name: "@mirai/google-gemini-cli-auth",
    });

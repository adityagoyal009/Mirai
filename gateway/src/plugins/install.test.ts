...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `mirai-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        mirai: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function createZipperArchiveBuffer(): Promise<Buffer> {
  ...
  zip.file(
    ...
    JSON.stringify({
      name: "@mirai/zipper",
      version: "0.0.1",
      mirai: { extensions: ["./dist/index.js"] },
    }),
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.mirai/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without mirai.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@mirai/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("mirai.extensions");
  });
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.mirai).toBeUndefined();
    expect(manifest.devDependencies?.vitest).toBe("^3.0.0");
  });

  it("uses mirai.plugin.json id as install key when it differs from package name", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@mirai/cognee-mirai",
        version: "0.0.1",
        mirai: { extensions: ["./dist/index.js"] },
      }),
      "utf-8",
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "cognee-mirai"',
        ),
      ),
    ).toBe(true);
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@mirai/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@mirai/voice-call@0.0.1",
    });

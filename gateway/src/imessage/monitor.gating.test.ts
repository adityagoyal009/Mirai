...
function baseCfg(): MiraiConfig {
  return {
    ...
    messages: {
      groupChat: { mentionPatterns: ["@mirai"] },
    },
...
function buildDispatchContextPayload(params: { cfg: MiraiConfig; message: IMessagePayload }) {
...
describe("imessage monitor gating + envelope builders", () => {
  ...
  it("blocks group messages when imessage.groups is set without a wildcard", () => {
    ...
    const decision = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@mirai hello",
      bodyText: "@mirai hello",
      allowFrom: ["*"],
      groupAllowFrom: [],
      groupPolicy: "open",
      dmPolicy: "open",

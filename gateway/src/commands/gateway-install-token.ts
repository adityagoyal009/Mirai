...
import { readConfigFileSnapshot, writeConfigFile, type MiraiConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("mirai config set gateway.auth.mode token")} or ${formatCliCommand("mirai config set gateway.auth.mode password")}.`,
  ].join(" ");
}

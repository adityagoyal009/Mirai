  ...
  if (resolved.provider !== "auto") {
    if (resolved.provider === "local") {
      ...
      note(
        [
          ...
          `- Switch to a remote provider: ${formatCliCommand("mirai config set agents.defaults.memorySearch.provider openai")}`,
          "",
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("mirai configure --section model")}`,
        `- To disable: ${formatCliCommand("mirai config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("mirai memory status --deep")}`,
      ].join("\n"),
      "Memory search",

...
export async function promptGatewayConfig(
  ...
  if (authMode === "token") {
    ...
    if (tokenInputMode === "ref") {
      ...
      note(`Validated ${envVarName}. Mirai will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Mirai trusts user identity from a reverse proxy.",
        ...
        "Docs: https://github.com/adityagoyal009/Mirai/tree/main/gateway/docs/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );

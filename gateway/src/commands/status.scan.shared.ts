...
export function hasExplicitMemorySearchConfig(cfg: MiraiConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: MiraiConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: MiraiConfig, agentId: string) => { store: { path: string } } | null;

...
export function resolveProviderAuthLoginCommand(params: {
  ...
  return formatCliCommand(`mirai models auth login --provider ${provider.id}`);
}
...
export function buildProviderAuthRecoveryHint(params: {
  ...
  if (params.includeConfigure !== false) {
    parts.push(`\`${formatCliCommand("mirai configure")}\``);
  }
  if (params.includeEnvVar) {
    parts.push("set an API key env var");
  }
  if (parts.length === 0) {
    return `Run \`${formatCliCommand("mirai configure")}\`.`;
  }
  if (parts.length === 1) {
    return `${parts[0]}.`;
  }

...
function createTelegramCfg(botToken: string, enabled?: boolean): MiraiConfig {
  return {
    channels: {
      telegram: {
        botToken,
...
function patchTelegramAdapter(overrides: Parameters<typeof patchChannelSetupWizardAdapter>[1]) {
  return patchChannelSetupWizardAdapter("telegram", {
    ...
    getStatus:
      overrides.getStatus ??
      vi.fn(async ({ cfg }: { cfg: MiraiConfig }) => ({
        channel: "telegram",
...
async function runQuickstartTelegramSetupWithInteractive(params: {
  ...
  try {
    const cfg = await runSetupChannels({} as MiraiConfig, prompter, {
      quickstartDefaults: true,
      onSelection: selection,
      onAccountId,
    });
...
vi.mock("./channel-setup/plugin-install.js", async (importOriginal) => {
  ...
  return {
    ...
    ensureChannelSetupPluginInstalled: vi.fn(async ({ cfg }: { cfg: MiraiConfig }) => ({
      cfg,
      installed: true,
    })),
...
describe("setupChannels", () => {
  ...
  it("QuickStart uses single-select (no multiselect) and doesn't prompt for Telegram token when WhatsApp is chosen", async () => {
    ...
    await runSetupChannels({} as MiraiConfig, prompter, {
      quickstartDefaults: true,
  ...
  it("renders the QuickStart channel picker without requiring the LINE runtime", async () => {
    ...
    await expect(
      runSetupChannels({} as MiraiConfig, prompter, {
        quickstartDefaults: true,
      }),
    ).resolves.toEqual({} as MiraiConfig);
    ...
    await runSetupChannels(
      {
        ...
        plugins: {
          entries: {
            "@mirai/msteams-plugin": { enabled: true },
          },
        },
      } as MiraiConfig,
      prompter,
        ...
        if (channel === "msteams") {
          registry.channels.push({
            ...
            plugin: {
              ...
              capabilities: { chatTypes: ["direct"] },
              config: {
                listAccountIds: (cfg: MiraiConfig) =>
                  Object.keys(
                    (cfg.channels?.msteams as { accounts?: Record<string, unknown> } | undefined)
                      ?.accounts ?? {},
                  ),
                resolveAccount: (cfg: MiraiConfig, accountId: string) =>
                  (
                    cfg.channels?.msteams as
                      | {
                          accounts?: Record<string, Record<string, unknown>>;
              ...
              setupWizard: {
                ...
                status: {
                  ...
                  resolveConfigured: ({ cfg }: { cfg: MiraiConfig }) =>
  ...
  it("applies configureInteractive result cfg/account updates", async () => {
    const configureInteractive = vi.fn(async ({ cfg }: { cfg: MiraiConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,
  ...
  it("uses configureWhenConfigured when channel is already configured", async () => {
    const configureWhenConfigured = vi.fn(async ({ cfg }: { cfg: MiraiConfig }) => ({
      cfg: {
        ...cfg,
        channels: {
          ...cfg.channels,

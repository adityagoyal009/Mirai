...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-mirai-backup";
...
describe("backupVerifyCommand", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("mirai-backup-verify-test-");
  });
  ...
  it("verifies an archive created by backup create", async () => {
    const stateDir = path.join(tempHome.home, ".mirai");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "mirai-backup-verify-out-"));
    try {
      await fs.writeFile(path.join(stateDir, "mirai.json"), JSON.stringify({}), "utf8");
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mirai-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mirai-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-mirai-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.mirai",
            archivePath: `${rootName}/payload/posix/tmp/.mirai`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("fails when archive paths contain traversal segments", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "mirai-backup-traversal-",
  ...
  it("fails when archive paths contain backslashes", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "mirai-backup-backslash-",
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    const stateDir = path.join(tempHome.home, ".mirai");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "mirai-workspace-"));
  ...
  it("fails when the archive contains duplicate root manifest entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.mirai/payload.txt`;
    await withBrokenArchiveFixture(
      {
        tempPrefix: "mirai-backup-duplicate-manifest-",
  ...
  it("fails when the archive contains duplicate payload entries", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "mirai-backup-duplicate-payload-",
        manifestAssetArchivePath: payloadArchivePath,
        payloads: [

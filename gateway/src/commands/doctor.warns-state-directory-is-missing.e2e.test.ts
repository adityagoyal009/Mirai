...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "mirai-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.MIRAI_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when MIRAI_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.MIRAI_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
  ...
  it("warns when token and password are both configured and gateway.auth.mode is unset", async () => {
    ...
    expect(String(gatewayAuthNote?.[0])).toContain("mirai config set gateway.auth.mode token");
    expect(String(gatewayAuthNote?.[0])).toContain(
      "mirai config set gateway.auth.mode password",
    );
  });
  ...
  it("keeps doctor read-only when gateway token is SecretRef-managed but unresolved", async () => {
    ...
    const previousToken = process.env.MIRAI_GATEWAY_TOKEN;
    delete process.env.MIRAI_GATEWAY_TOKEN;
    note.mockClear();
    try {
      await doctorCommand(createDoctorRuntime(), {
      ...
      if (previousToken === undefined) {
        ...
        process.env.MIRAI_GATEWAY_TOKEN = previousToken;
      }
    }

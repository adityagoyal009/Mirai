...
describe("resetCommand", () => {
  ...
  beforeEach(() => {
    ...
    resolveCleanupPlanFromDisk.mockReturnValue({
      stateDir: "/tmp/.mirai",
      configPath: "/tmp/.mirai/mirai.json",
      oauthDir: "/tmp/.mirai/credentials",
      configInsideState: true,
      oauthInsideState: true,
      workspaceDirs: ["/tmp/.mirai/workspace"],
    });
    removePath.mockResolvedValue({ ok: true });
    listAgentSessionDirs.mockResolvedValue(["/tmp/.mirai/agents/main/sessions"]);
  ...
  it("recommends creating a backup before state-destructive reset scopes", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(expect.stringContaining("mirai backup create"));
  });
  ...
  it("does not recommend backup for config-only reset", async () => {
    ...
    expect(runtime.log).not.toHaveBeenCalledWith(expect.stringContaining("mirai backup create"));
  });
});

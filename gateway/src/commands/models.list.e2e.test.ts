...
const ensureMiraiModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveMiraiAgentDir = vi.fn().mockReturnValue("/tmp/mirai-agent");
...
beforeEach(() => {
  ...
  ensureMiraiModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureMiraiModelsJson).toHaveBeenCalled();
    expect(ensureMiraiModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });

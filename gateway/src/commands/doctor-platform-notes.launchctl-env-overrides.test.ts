...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "MIRAI_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("MIRAI_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv MIRAI_GATEWAY_TOKEN");
    expect(message).not.toContain("MIRAI_GATEWAY_PASSWORD");
  });

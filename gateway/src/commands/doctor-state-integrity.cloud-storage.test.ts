...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Mirai", ".mirai");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/mirai-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".mirai");
  ...
  it("anchors cloud detection to OS homedir when MIRAI_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".mirai");
    const originalMiraiHome = process.env.MIRAI_HOME;
    process.env.MIRAI_HOME = "/tmp/mirai-home-override";

...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Mirai status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`mirai devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("mirai devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("mirai devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("mirai security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("mirai security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("mirai status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("mirai logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("mirai status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("mirai gateway probe")}`);
  }
}

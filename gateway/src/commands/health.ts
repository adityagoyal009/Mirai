...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.MIRAI_DEBUG_HEALTH)) {
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: MiraiConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.MIRAI_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {

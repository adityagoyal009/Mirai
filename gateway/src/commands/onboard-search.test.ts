...
function createPerplexityConfig(apiKey: string, enabled?: boolean): MiraiConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: MiraiConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): MiraiConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: MiraiConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: MiraiConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);

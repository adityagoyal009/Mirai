    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("mirai configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("mirai config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("mirai doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "mirai config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("mirai config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('mirai config set session.dmScope "per-channel-peer"') +

...
function resolveAttachmentLimits(config: MiraiConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".mirai", "attachments");
  const relDir = path.posix.join(".mirai", "attachments", attachmentId);

...
const GOOGLE_LIVE_SINGLE_KEY = "MIRAI_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "MIRAI_LIVE_ANTHROPIC_KEY",
    listVar: "MIRAI_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "MIRAI_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `MIRAI_LIVE_${base}_KEY`;

...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses MIRAI_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      MIRAI_HOME: "/srv/mirai-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/mirai-home"), ".mirai", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".mirai", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("mirai-workspace-");

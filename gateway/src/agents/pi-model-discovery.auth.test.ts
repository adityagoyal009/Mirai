...
async function createAgentDir(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "mirai-pi-auth-storage-"));
}
...
describe("discoverAuthStorage", () => {
  ...
  it("preserves legacy auth.json when auth store is forced read-only", async () => {
    await withAgentDir(async (agentDir) => {
      const previous = process.env.MIRAI_AUTH_STORE_READONLY;
      process.env.MIRAI_AUTH_STORE_READONLY = "1";
      try {
        writeRuntimeOpenRouterProfile(agentDir);
        await writeLegacyAuthJson(agentDir, {
          openrouter: { type: "api_key", key: "legacy-static-key" },
        ...
        if (previous === undefined) {
          delete process.env.MIRAI_AUTH_STORE_READONLY;
        } else {
          process.env.MIRAI_AUTH_STORE_READONLY = previous;
        }
      }
    });
  });

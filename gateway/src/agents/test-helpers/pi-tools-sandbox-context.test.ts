...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("mirai-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("mirai-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("mirai-sbx-");

...
function makeFallbacksOnlyCfg(): MiraiConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): MiraiConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
describe("runWithModelFallback", () => {
  ...
  describe("fallback behavior with provider cooldowns", () => {
    async function makeAuthStoreWithCooldown(
      ...
      const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mirai-test-"));
      const now = Date.now();
      const store: AuthProfileStore = {
        version: AUTH_STORE_VERSION,
        profiles: {

    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "mirai.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "mirai.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "mirai.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("mirai-");
  const acpxRoot = await tempDirs.make("mirai-acpx-plugin-");
  const helperRoot = await tempDirs.make("mirai-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("mirai-");
  const pluginRoot = await tempDirs.make("mirai-plugin-");
  const outsideDir = await tempDirs.make("mirai-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("mirai-");
    const pluginRoot = await tempDirs.make("mirai-claude-bundle-");

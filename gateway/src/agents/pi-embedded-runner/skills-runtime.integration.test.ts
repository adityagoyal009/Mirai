...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("mirai-bundled-");
  const workspaceDir = await createTempDir("mirai-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: MiraiConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.MIRAI_BUNDLED_PLUGINS_DIR = bundledPluginsDir;

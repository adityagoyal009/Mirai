...
function createOpenAiApiKeySourceConfig(): MiraiConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiApiKeyRuntimeConfig(): MiraiConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderSourceConfig(): MiraiConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderRuntimeConfig(): MiraiConfig {
  return {
    models: {
      providers: {
        openai: {
...
function withGatewayTokenMode(config: MiraiConfig): MiraiConfig {
  return {
    ...config,
    gateway: {
      auth: {
...
async function withGeneratedModelsFromRuntimeSource(
  params: {
    ...
    runtimeConfig: MiraiConfig;
    candidateConfig?: MiraiConfig;
  },
  ...
  await withTempHome(async () => {
    try {
      ...
      await ensureMiraiModelsJson(params.candidateConfig ?? loadConfig());
      await runAssertions();
    } finally {
      clearRuntimeConfigSnapshot();
      clearConfigCache();
...
describe("models-config runtime source snapshot", () => {
  ...
  it("uses non-env marker from runtime source snapshot for file refs", async () => {
    await withTempHome(async () => {
      const sourceConfig: MiraiConfig = {
        models: {
          providers: {
            moonshot: {
      ...
      const runtimeConfig: MiraiConfig = {
        models: {
          providers: {
            moonshot: {
              baseUrl: "https://api.moonshot.ai/v1",
      ...
      try {
        ...
        await ensureMiraiModelsJson(loadConfig());
  ...
  it("projects cloned runtime configs onto source snapshot when preserving provider auth", async () => {
    await withTempHome(async () => {
      ...
      const clonedRuntimeConfig: MiraiConfig = {
        ...runtimeConfig,
        agents: {
          defaults: {
      ...
      try {
        setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
        await ensureMiraiModelsJson(clonedRuntimeConfig);
  ...
  it("keeps source markers when runtime projection is skipped for incompatible top-level shape", async () => {
    await withTempHome(async () => {
      ...
      const incompatibleCandidate: MiraiConfig = {
        ...createOpenAiApiKeyRuntimeConfig(),
      };

      try {
        setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
        await ensureMiraiModelsJson(incompatibleCandidate);
        await expectGeneratedProviderApiKey("openai", "OPENAI_API_KEY"); // pragma: allowlist secret
      } finally {
        clearRuntimeConfigSnapshot();
        clearConfigCache();

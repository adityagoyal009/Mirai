...
const defaultTools = createMiraiCodingTools({ senderIsOwner: true });
...
describe("createMiraiCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: MiraiConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createMiraiCodingTools({
    ...
    const anthropicTools = createMiraiCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: MiraiConfig = {
      tools: {
        exec: {
    ...
    const allowed = createMiraiCodingTools({
    ...
    const denied = createMiraiCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createMiraiCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });

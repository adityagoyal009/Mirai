...
let previousConfigPath = process.env.MIRAI_CONFIG_PATH;
...
describe("spawnSubagentDirect filename validation", () => {
  beforeEach(() => {
    ...
    workspaceDirOverride = fs.mkdtempSync(
      path.join(os.tmpdir(), `mirai-subagent-attachments-${process.pid}-${Date.now()}-`),
    );
    configPathOverride = path.join(workspaceDirOverride, "mirai.test.json");
    fs.writeFileSync(configPathOverride, JSON.stringify(configOverride, null, 2));
    previousConfigPath = process.env.MIRAI_CONFIG_PATH;
    process.env.MIRAI_CONFIG_PATH = configPathOverride;
  });
  ...
  afterEach(() => {
    if (previousConfigPath === undefined) {
      ...
      process.env.MIRAI_CONFIG_PATH = previousConfigPath;
    }
    configPathOverride = "";
  ...
  it("removes materialized attachments when lineage patching fails", async () => {
    ...
    const attachmentsRoot = path.join(workspaceDirOverride, ".mirai", "attachments");

...
async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "mirai-pdf-"));
  try {
...
function withDefaultModel(primary: string): MiraiConfig {
  return {
    agents: { defaults: { model: { primary } } },
  } as MiraiConfig;
}

function withPdfModel(primary: string): MiraiConfig {
  return {
...
async function stubPdfToolInfra(
  ...
  vi.spyOn(modelsConfig, "ensureMiraiModelsJson").mockResolvedValue({
    agentDir,
    wrote: false,
  });
...
describe("createPdfTool", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox pdf paths", async () => {
    await withTempAgentDir(async (agentDir) => {
      vi.stubEnv("ANTHROPIC_API_KEY", "anthropic-test");
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "mirai-pdf-ws-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "mirai-pdf-out-"));
      try {
        const cfg = withDefaultModel(ANTHROPIC_PDF_MODEL);

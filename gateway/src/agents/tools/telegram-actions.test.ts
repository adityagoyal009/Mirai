...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): MiraiConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as MiraiConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): MiraiConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as MiraiConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: MiraiConfig, accountId = "media") {
    await handleTelegramAction(

...
function findTool(name: string, config: MiraiConfig) {
  const allTools = createMiraiTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: MiraiConfig; env?: NodeJS.ProcessEnv }) {
...
describe("mirai tools runtime web metadata wiring", () => {

...
function buildCoreToolGroupMap() {
  ...
  const miraiTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInMiraiGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:mirai": miraiTools,

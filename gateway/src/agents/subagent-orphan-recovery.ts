 ...
 *
 * @see https://github.com/mirai/mirai/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /mirai\.json|mirai gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumed = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify mirai.json or restart the gateway]"
            : undefined,
          originalRunId: runId,

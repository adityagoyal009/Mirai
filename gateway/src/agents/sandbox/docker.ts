...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "mirai.sandbox=1");
  args.push("--label", `mirai.sessionKey=${params.scopeKey}`);
  args.push("--label", `mirai.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `mirai.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "mirai.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`mirai sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`mirai sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("mirai sandbox recreate --all");
}

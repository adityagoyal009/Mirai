...
export async function createSshSandboxSessionFromConfigText(params: {
  ...
  const configDir = await fs.mkdtemp(path.join(resolveSshTmpRoot(), "mirai-sandbox-ssh-"));
...
function resolveSshTmpRoot(): string {
  return path.resolve(resolvePreferredMiraiTmpDir() ?? os.tmpdir());
}

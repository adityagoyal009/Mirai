...
function createConfig(): MiraiConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "mirai-test-ssh-config"),
    host: "mirai-sandbox",
  };
}
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "mirai-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "mirai-ssh-worker-abcd1234",
    ...
    expect(sshMocks.runSshSandboxCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        remoteCommand: expect.stringContaining("/remote/mirai/mirai-ssh-agent-worker"),
      }),
    );
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/mirai/mirai-ssh-agent-worker");

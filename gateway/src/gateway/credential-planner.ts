...
type GatewaySecretDefaults = NonNullable<MiraiConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${MIRAI_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(
  ...
  return readGatewayEnv(
    env,
    ["MIRAI_GATEWAY_TOKEN", "MIRAI_LEGACY_GATEWAY_TOKEN"],
    includeLegacyEnv,
  );
}
...
export function readGatewayPasswordEnv(
  ...
  return readGatewayEnv(
    env,
    ["MIRAI_GATEWAY_PASSWORD", "MIRAI_LEGACY_GATEWAY_PASSWORD"],
    includeLegacyEnv,
  );
}

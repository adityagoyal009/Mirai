...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.MIRAI_SKIP_CHANNELS;
    prevSkipGmail = process.env.MIRAI_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.MIRAI_SKIP_PROVIDERS;
    process.env.MIRAI_SKIP_CHANNELS = "0";
    delete process.env.MIRAI_SKIP_GMAIL_WATCHER;
    delete process.env.MIRAI_SKIP_PROVIDERS;
  });
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.MIRAI_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.MIRAI_SKIP_GMAIL_WATCHER;
    } else {
      process.env.MIRAI_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.MIRAI_SKIP_PROVIDERS;
    } else {
      process.env.MIRAI_SKIP_PROVIDERS = prevSkipProviders;
    }
  });

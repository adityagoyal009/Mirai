...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mirai-session-history-"));
  ...
  test("supports cursor pagination over direct REST while preserving the messages field", async () => {
    ...
    try {
      ...
      const firstBody = (await firstPage.json()) as {
        sessionKey?: string;
        items?: Array<{ content?: Array<{ text?: string }>; __mirai?: { seq?: number } }>;
        messages?: Array<{ content?: Array<{ text?: string }>; __mirai?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(firstBody.messages?.map((message) => message.__mirai?.seq)).toEqual([2, 3]);
      ...
      const secondBody = (await secondPage.json()) as {
        items?: Array<{ content?: Array<{ text?: string }>; __mirai?: { seq?: number } }>;
        messages?: Array<{ __mirai?: { seq?: number } }>;
        nextCursor?: string;
        hasMore?: boolean;
      };
      ...
      expect(secondBody.messages?.map((message) => message.__mirai?.seq)).toEqual([1]);
  ...
  test("streams session history updates over SSE", async () => {
    ...
    try {
      ...
      expect(
        (
          messageEvent.data as {
            message?: { __mirai?: { id?: string; seq?: number } };
          }
        ).message?.__mirai,
      ).toMatchObject({

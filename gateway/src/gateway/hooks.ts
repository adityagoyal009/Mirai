...
export function resolveHooksConfig(cfg: MiraiConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: MiraiConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-mirai-token"] === "string"
      ? req.headers["x-mirai-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }

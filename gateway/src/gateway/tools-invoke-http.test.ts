...
let lastCreateMiraiToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/mirai-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateMiraiToolsContext?.agentTo,
          agentThreadId: lastCreateMiraiToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createMiraiTools: (ctx: Record<string, unknown>) => {
      lastCreateMiraiToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateMiraiToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-mirai-message-to": "channel:24514",
        "x-mirai-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });

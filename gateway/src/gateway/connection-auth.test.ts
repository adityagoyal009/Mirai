...
function cfg(input: Partial<MiraiConfig>): MiraiConfig {
  return input as MiraiConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: MiraiConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
...
const DEFAULT_ENV = {
  ...
  MIRAI_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  const cases: ConnectionAuthCase[] = [
    ...
    },
    {
      ...
      env: {
        MIRAI_LEGACY_GATEWAY_TOKEN: "legacy-token",
        MIRAI_LEGACY_GATEWAY_PASSWORD: "legacy-password", // pragma: allowlist secret
      } as NodeJS.ProcessEnv,
      options: {
  ...
  it("resolves config-first token SecretRef even when MIRAI env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when MIRAI env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {

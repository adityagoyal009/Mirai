...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("prefers MIRAI_HANDSHAKE_TIMEOUT_MS and falls back on empty string", () => {
      const prevHandshakeTimeout = process.env.MIRAI_HANDSHAKE_TIMEOUT_MS;
      const prevTestHandshakeTimeout = process.env.MIRAI_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.MIRAI_HANDSHAKE_TIMEOUT_MS = "75";
      process.env.MIRAI_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        expect(getHandshakeTimeoutMs()).toBe(75);
        process.env.MIRAI_HANDSHAKE_TIMEOUT_MS = "";
        expect(getHandshakeTimeoutMs()).toBe(20);
      } finally {
        if (prevHandshakeTimeout === undefined) {
          delete process.env.MIRAI_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.MIRAI_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
        if (prevTestHandshakeTimeout === undefined) {
          delete process.env.MIRAI_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.MIRAI_TEST_HANDSHAKE_TIMEOUT_MS = prevTestHandshakeTimeout;
        }
      }
    });

...
import type { MiraiConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: MiraiConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: MiraiConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: MiraiConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: MiraiConfig) => void;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: MiraiConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: MiraiConfig) => {

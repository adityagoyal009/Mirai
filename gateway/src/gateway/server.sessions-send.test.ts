...
const { createMiraiTools } = await import("../agents/mirai-tools.js");
...
type SessionSendTool = ReturnType<typeof createMiraiTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createMiraiTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["MIRAI_GATEWAY_PORT", "MIRAI_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.MIRAI_GATEWAY_PORT = String(gatewayPort);
  process.env.MIRAI_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("MIRAI_CONFIG_PATH missing in gateway test environment");
      }

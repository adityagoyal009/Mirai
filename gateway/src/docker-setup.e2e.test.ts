...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "mirai-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    MIRAI_GATEWAY_TOKEN: "test-token",
    MIRAI_CONFIG_DIR: join(sandbox.rootDir, "config"),
    MIRAI_WORKSPACE_DIR: join(sandbox.rootDir, "mirai"),
  };
...
async function runDockerSetupWithUnsetGatewayToken(
  ...
  const result = runDockerSetup(sandbox, {
    MIRAI_GATEWAY_TOKEN: undefined,
    MIRAI_CONFIG_DIR: configDir,
    MIRAI_WORKSPACE_DIR: workspaceDir,
  });
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      MIRAI_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      MIRAI_EXTRA_MOUNTS: undefined,
      MIRAI_HOME_VOLUME: "mirai-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MIRAI_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("MIRAI_EXTRA_MOUNTS=");
    expect(envFile).toContain("MIRAI_HOME_VOLUME=mirai-home"); // pragma: allowlist secret
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("mirai-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("mirai-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg MIRAI_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(log).toContain("run --rm mirai-cli onboard --mode local --no-install-daemon");
    expect(log).toContain("run --rm mirai-cli config set gateway.mode local");
    expect(log).toContain("run --rm mirai-cli config set gateway.bind lan");
  });
  ...
  it("writes MIRAI_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      MIRAI_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MIRAI_TZ=Asia/Shanghai");
  });
  ...
  it("reuses existing config token when MIRAI_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("MIRAI_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when MIRAI_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "MIRAI_GATEWAY_TOKEN=dotenv-token-123\nMIRAI_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("MIRAI_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        "MIRAI_GATEWAY_TOKEN=",
        "MIRAI_GATEWAY_TOKEN=first-token",
        "MIRAI_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("MIRAI_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("MIRAI_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats MIRAI_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MIRAI_SANDBOX=");

    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg MIRAI_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg MIRAI_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      MIRAI_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker mirai-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        MIRAI_SANDBOX: "1",
        MIRAI_DOCKER_SOCKET: socketPath,
      ...
      const gatewayStarts = log
        ...
        .filter(
          (line) =>
            ...
            line.includes(" up -d") &&
            line.includes("mirai-gateway"),
        );
      expect(gatewayStarts).toHaveLength(2);
      expect(log).toContain(
        "run --rm --no-deps mirai-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate mirai-gateway"));
  ...
  it("rejects injected multiline MIRAI_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      MIRAI_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("MIRAI_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid MIRAI_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      MIRAI_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid MIRAI_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      MIRAI_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("MIRAI_HOME_VOLUME must match");
  });

  it("rejects MIRAI_TZ values that are not present in zoneinfo", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("MIRAI_TZ must match a timezone in /usr/share/zoneinfo");
  });
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:mirai-gateway"');
    expect(compose).toContain("depends_on:\n      - mirai-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/MIRAI_GATEWAY_TOKEN: \$\{MIRAI_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{MIRAI_TZ:-UTC\}/g)).toHaveLength(2);
  });
});

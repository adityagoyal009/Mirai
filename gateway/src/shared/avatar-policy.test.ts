...
describe("avatar policy", () => {
  it("accepts workspace-relative avatar paths and rejects URI schemes", () => {
    expect(isWorkspaceRelativeAvatarPath("avatars/mirai.png")).toBe(true);
    expect(isWorkspaceRelativeAvatarPath("C:\\\\avatars\\\\mirai.png")).toBe(true);
  ...
  it("detects avatar-like path strings", () => {
    expect(looksLikeAvatarPath("avatars/mirai.svg")).toBe(true);
    expect(looksLikeAvatarPath("mirai.webp")).toBe(true);

...
const RELAY_AUTH_HEADER = "x-mirai-relay-token";
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/Mirai-Extension-Relay",
          revision: "0",
          userAgent: "Mirai-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  try {
    ...
    if (
      ...
      (await probeAuthenticatedMiraiRelay({

...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["MIRAI_GATEWAY_TOKEN"]);
    process.env.MIRAI_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
  });
  ...
  it("uses relay-scoped token only for known relay ports", async () => {
    ...
    expect(Object.keys(headers)).toContain("x-mirai-relay-token");
    expect(headers["x-mirai-relay-token"]).not.toBe(TEST_GATEWAY_TOKEN);
  });
  ...
  it("accepts extension websocket access with relay token query param", async () => {
    ...
    const token = relayAuthHeaders(`ws://127.0.0.1:${port}/extension`)["x-mirai-relay-token"];
  ...
  it("accepts raw gateway token for relay auth compatibility", async () => {
    ...
    const versionRes = await fetch(`${cdpUrl}/json/version`, {
      headers: { "x-mirai-relay-token": TEST_GATEWAY_TOKEN },
    });
  ...
  it("reuses an already-bound relay port when another process owns it", async () => {
    ...
    const fakeRelay = createServer((req, res) => {
      if (req.url?.startsWith("/json/version")) {
        const header = req.headers["x-mirai-relay-token"];
        ...
        res.end(JSON.stringify({ Browser: "Mirai/extension-relay" }));
        return;
      }
  ...
  it("does not swallow EADDRINUSE when occupied port is not an mirai relay", async () => {
    const port = await getFreePort();

import type { BrowserConfig, BrowserProfileConfig, MiraiConfig } from "../config/config.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_MIRAI_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "mirai" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_MIRAI_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_MIRAI_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Mirai browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for mirai/mirai CDP).
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_MIRAI_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "mirai";

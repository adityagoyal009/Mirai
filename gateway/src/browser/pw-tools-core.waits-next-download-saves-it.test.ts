...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredMiraiTmpDir: vi.fn(() => "/tmp/mirai"),
}));
vi.mock("../infra/tmp-mirai-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredMiraiTmpDir.mockReturnValue("/tmp/mirai");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredMiraiTmpDir.mockReturnValue("/tmp/mirai-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "mirai-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredMiraiTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "mirai-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "mirai-preferred", "downloads")}${path.sep}`),
    );
  });

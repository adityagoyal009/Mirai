...
describe("browser server-context remote profile tab operations", () => {
  it("uses profile-level attachOnly when global attachOnly is false", async () => {
    ...
    const launchMock = vi.mocked(chromeModule.launchMiraiChrome);
    const ctx = createBrowserRouteContext({ getState: () => state });

    await expect(ctx.forProfile("mirai").ensureBrowserAvailable()).rejects.toThrow(
  ...
  it("does not enforce managed tab cap for remote mirai profiles", async () => {

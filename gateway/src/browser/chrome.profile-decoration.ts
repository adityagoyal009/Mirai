...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".mirai-profile-decorated");
}
 ...
 */
export function decorateMiraiProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_MIRAI_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MIRAI_BROWSER_COLOR).toUpperCase();

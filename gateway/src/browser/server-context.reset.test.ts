...
function localMiraiProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "mirai",
...
function createLocalMiraiResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localMiraiProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveMiraiUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mirai-reset-"));
    const profileDir = path.join(tempRoot, "mirai");
    ...
    const ops = createLocalMiraiResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveMiraiUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mirai-reset-no-own-"));
    const profileDir = path.join(tempRoot, "mirai");
    fs.mkdirSync(profileDir, { recursive: true });

...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.MIRAI_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/mirai" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          mirai: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchMiraiChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.MIRAI_GATEWAY_PORT;
  process.env.MIRAI_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.MIRAI_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.MIRAI_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.MIRAI_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.MIRAI_GATEWAY_PASSWORD;
  } else {
    process.env.MIRAI_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}

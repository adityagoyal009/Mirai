...
describe("canvas host state dir defaults", () => {
  it("uses MIRAI_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("mirai-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Mirai Canvas");
      } finally {
        await handler.close();
      }
    });

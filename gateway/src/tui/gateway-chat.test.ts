...
describe("resolveGatewayConnection", () => {
  ...
  it("uses MIRAI_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ MIRAI_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers MIRAI_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ MIRAI_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});

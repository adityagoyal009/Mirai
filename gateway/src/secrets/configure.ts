...
function getSecretProviders(config: MiraiConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: MiraiConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function toSourceChoices(config: MiraiConfig): Array<{ value: SecretRefSource; label: string }> {
...
function configureCandidateKey(candidate: {
  configFile: "mirai.json" | "auth-profiles.json";
  path: string;
  agentId?: string;
}): string {
...
function resolveConfigureAgentId(config: MiraiConfig, explicitAgentId?: string): string {
...
async function configureProvidersInteractive(config: MiraiConfig): Promise<void> {
  while (true) {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    const candidates = buildConfigureCandidatesForScope({
      config: stagedConfig,
      authoredMiraiConfig: snapshot.resolved,
      authProfiles: {
        agentId: configureAgentId,
        store: authStore,
      },
    ...
    while (true) {
      ...
      const options = visibleCandidates.map((candidate) => ({
        ...
        hint: [
          candidate.configFile === "auth-profiles.json" ? "auth-profiles.json" : "mirai.json",

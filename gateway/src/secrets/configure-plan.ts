...
export function buildConfigureCandidates(config: MiraiConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  config: MiraiConfig;
  authoredMiraiConfig?: MiraiConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredMiraiConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const miraiCandidates = discoverConfigSecretTargets(params.config)
    ...
    .map((entry) => {
      ...
      return {
        ...
        label: entry.path,
        configFile: "mirai.json" as const,
  ...
  return [...miraiCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "mirai secrets configure",

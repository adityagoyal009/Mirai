...
const MIRAI_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of MIRAI_COMPILED_SECRET_TARGETS) {
...
const MIRAI_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: MIRAI_COMPILED_SECRET_TARGETS,
    entriesById: MIRAI_TARGETS_BY_ID,
  });

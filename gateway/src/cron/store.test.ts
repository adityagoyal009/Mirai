...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mirai-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses MIRAI_HOME for tilde expansion", () => {
    vi.stubEnv("MIRAI_HOME", "/srv/mirai-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/mirai-home", "cron", "jobs.json"));
  });
});

...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `mirai: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["mirai: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:adityagoyal009/Mirai#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, build from source: `cd gateway && pnpm install && pnpm build`.");
  return lines.join("\n");
};

...
function browserMockAssetsPlugin(): Plugin {
  ...
  return {
    ...
    closeBundle() {
      ...
      // Pre-decode sprites for standalone browser deployment
      const decodedDir = path.join(distAssetsDir, 'decoded');
      fs.mkdirSync(decodedDir, { recursive: true });
      try {
        fs.writeFileSync(path.join(decodedDir, 'characters.json'), JSON.stringify(decodeAllCharacters(assetsDir)));
        fs.writeFileSync(path.join(decodedDir, 'floors.json'), JSON.stringify(decodeAllFloors(assetsDir)));
        fs.writeFileSync(path.join(decodedDir, 'walls.json'), JSON.stringify(decodeAllWalls(assetsDir)));
        fs.writeFileSync(path.join(decodedDir, 'furniture.json'), JSON.stringify(decodeAllFurniture(assetsDir, catalog)));
        console.log('[build] Decoded sprite assets written to dist/assets/decoded/');
      } catch (e) {
        console.warn('[build] Failed to decode some sprite assets:', e);
      }
    },
  };
}

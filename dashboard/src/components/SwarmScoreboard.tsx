...
export function SwarmScoreboard({ isMobile = false, onClose, hidden = false }: { isMobile?: boolean; onClose?: () => void; hidden?: boolean } = {}) {
  ...
  useEffect(() => {
    ...
    const unsub = mirai.onMessage((msg) => {
      ...
      if (mt === 'analysisComplete') { setState(prev => ({ ...prev, running: false, phase: 'complete', fullResult: (msg as any).fullResult, reportReady: false })); return; }
      if (mt === 'reportGenerating') { setState(prev => ({ ...prev, reportReady: false })); return; }
      if (mt === 'reportReady') { setState(prev => ({ ...prev, reportReady: true, reportId: (msg as any).reportId })); return; }
      if (mt === 'reportFailed') { setState(prev => ({ ...prev, reportReady: undefined })); return; }
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              ...
              <div style={{
                ...
                <select value={agentCount} onChange={e => setAgentCount(Number(e.target.value))}
                  ...
                  <option value={50} style={{ background: '#0a0a18', color: '#e0e0e0' }}>50 agents</option>
                </select>
          ...
          {state.result && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                <button
                  onClick={() => {
                    // Open HTML report in new tab (report pre-generated by backend)
                    if (state.reportId) {
                      window.open(`/api/bi/report/html/${state.reportId}`, '_blank')
                    }
                  }}
                  disabled={state.phase !== 'complete' || (state.phase === 'complete' && state.reportReady === false)}
                  style={{
                    flex: 1, padding: '12px 0', borderRadius: 8, border: 'none', fontSize: 13, fontWeight: 700, letterSpacing: 1, cursor: (state.phase === 'complete' && state.reportReady !== false) ? 'pointer' : 'default',
                    background: (state.phase === 'complete' && state.reportReady !== false) ? 'linear-gradient(90deg, #4488ff, #6644ff)' : state.phase === 'complete' ? 'linear-gradient(90deg, #333, #444)' : '#111',
                    color: (state.phase === 'complete' && state.reportReady !== false) ? '#fff' : '#555',
                    boxShadow: (state.phase === 'complete' && state.reportReady !== false) ? '0 4px 16px rgba(68, 136, 255, 0.2)' : 'none',
                  }}>
                  {state.phase === 'complete' ? (state.reportReady === false ? 'GENERATING REPORT...' : 'VIEW REPORT') : 'GENERATING...'}
                </button>
                <button
